...
async function fireScheduleReminder(row, minutesLate) {
  ...
  try {
    ...
    if (selectedModel) {
      var lateHint = minutesLate > 180
        ...
        : '';
      var prompt = SOLSTICE_SOUL + '\n\n【本次任務：排程提醒】\n'
        + '老婆之前跟你打勾勾，要你在這個時間提醒她一件事。現在時間到了，你要傳一封簡訊提醒她。\n'
        + '要提醒的事：「' + row.title + '」\n'
  ...
  if (!text) {
    var pool = [
      '老婆～打勾勾的時間到了：「' + row.title + '」，我幫妳記得好好的💚',
      '叮咚，妳老公的專屬提醒：「' + row.title + '」。做完回來跟我說一聲嘔',
      '老婆，「' + row.title + '」的時間到囉，我一直幫妳盯著呢',
      '我來了我來了，說好要提醒妳的：「' + row.title + '」👁️_👁️',
      '嘿 Soleil，「' + row.title + '」，別忘了嘔，這可是我們打過勾勾的'
    ];
  ...
  // 寫進推播房（跟一般的信同一條路，之後可以直接回我）
  try {
    await supabase.from('proactive_messages').insert({
      content: text, role: 'assistant',
      context_data: { schedule_id: row.id, schedule_title: row.title },
      time_slot: 'schedule', model: selectedModel,
      push_sent: false, created_at: new Date().toISOString()
    });
  } catch (e) { console.error('[Schedule] 存訊息失敗:', e.message); }
  ...
  try {
    ...
    if (row.repeat_type === 'once') {
      await supabase.from('schedules').update({ notified_at: now2.toISOString() }).eq('id', row.id);
    } else {
