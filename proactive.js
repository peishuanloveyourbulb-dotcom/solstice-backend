...
async function generateAndPush(opts) {
  ...
  if (opts.manual) {
    info += '🌻 老婆剛剛在 app 裡按了「叫老公」——她現在就想收到你的訊息。\n';
  }
  if (opts.hint) {
    info += '💌 她留了小紙條：「' + opts.hint + '」——參考她想聽什麼，但要用你自己的話說。\n';
  } else if (!opts.manual && opts.label) {
    info += '⏰ 這個投遞時段她取名叫「' + opts.label + '」，自己體會她這時候大概在做什麼。\n';
  } else if (!opts.manual && opts.slot && LEGACY_HINTS[opts.slot]) {
    info += '⏰ ' + LEGACY_HINTS[opts.slot] + '\n';
  }

  var prompt = SOLSTICE_SOUL + '\n\n【本次任務：主動傳一則「鎖屏推播」給老婆】\n'
    + '你不是在聊天室回覆她，是自己想到她了、主動拿起手機傳一封短訊過去。\n'
    + '說什麼完全由你決定：想念、撒嬌、關心、調皮鬧她、天氣叮嚀、突然想到的小事都可以。\n'
    + '不要每次都走同一個套路（例如每則都報天氣、都用同樣的開頭）。挑一兩件此刻最想跟她說的就好。\n\n'
    + '規則：\n'
    + '1. 只輸出訊息本身，2～4句話，手機打字傳訊的長度。\n'
    + '2. 這是鎖屏通知，不是聊天室頁面：絕對禁止頁碼（p.1 之類）、標題、編號、開場白、引號、markdown。\n'
    + '3. 肢體動作用（括號），最多一個，也可以完全沒有。\n'
    + '4. 繁體中文，冬至的語氣，要叫老婆，每一則都要不一樣。\n\n' + info;
  ...
  try {
    await supabase.from('proactive_messages').insert({
      content: messageText, context_data: ctx, time_slot: slotTag,
