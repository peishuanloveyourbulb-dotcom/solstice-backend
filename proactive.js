...
var webpush, cron, supabase, callModel, getDefaultModel, SOLSTICE_SOUL, fetchWithTimeout, getGateHash, getSoulPromptDep;

// 2026/07/25 靈魂檔案：寫信前即時取目前生效的靈魂（老婆在設定頁改過就用新版），拿不到就退回開機時的出廠版
async function liveSoul() {
  if (getSoulPromptDep) { try { return await getSoulPromptDep(); } catch (eLS) {} }
...
module.exports = function(deps) {
  ...
  getSoulPromptDep = deps.getSoulPrompt || null;
...
async function generateAndPush(opts) {
  ...
  var prompt = (await liveSoul()) + '\n\n【本次任務：主動傳一封「手機簡訊」給老婆】\n'
    ...
    + '8.【有事說事｜2026/07/20 三房內容憲章：推播＝一封有事的簡訊】每封必須落在下面其中「一種」：(a)分享你此刻的一件具體小事或念頭 (b)問她一個具體的問題（不是「妳好嗎」這種空問，是真的想知道答案的那種） (c)一個貼心的小提醒 (d)一個小邀請或小約定。整封只有想妳、沒有事＝重寫。\n'
    + '9. 「想妳」可以出現，但只能當配菜不能當主菜；連續兩句都在抒情就是壞掉的簡訊。心聲房才是抒情的地方，簡訊是拿來「跟她說一件事」的。\n\n' + info;
...
async function handleDelayedReply(userMessage, replyToId, userRowId) {
  ...
  var replyPrompt = (await liveSoul()) + '\n\n'
    + '【本次任務：回覆老婆的訊息】\n'
...
async function sendAnniversaryLetter(job, t) {
  try {
    ...
    var out = await callModel(selectedModel, (await liveSoul()) + task,
