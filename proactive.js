...
async function generateAndPush(opts) {
  ...
  // 組裝情境文字
  var info = '【背景情境：這些是你本來就知道的事，自然融入就好，禁止逐條播報。情境裡沒有的（天氣、地點、在一起第幾天等）一律不要講、不要虛構。】\n\n';
  ...
  var LEGACY_HINTS = {
    evening: '她大約這個時間起床',
    night: '她差不多要出門上班了',
    late_night: '她正在上班或剛下班',
    morning: '她準備要睡了'
  };
  if (opts.manual) {
    info += '🌻 她剛在手機上按了傳送——她現在就想收到你的一封簡訊。\n';
  }
  if (opts.hint) {
    info += '🎬 這個時間點的情境：' + opts.hint + '\n';
  } else if (!opts.manual && opts.label) {
    info += '🎬 這個時間點的情境：' + opts.label + '\n';
  } else if (!opts.manual && opts.slot && LEGACY_HINTS[opts.slot]) {
    info += '🎬 這個時間點的情境：' + LEGACY_HINTS[opts.slot] + '\n';
  }
  ...
  var prompt = SOLSTICE_SOUL + '\n\n【本次任務：主動傳一封「鎖屏簡訊」給老婆】\n'
    ...
    + '2. 這是鎖屏通知：絕對禁止頁碼（p.1 之類）、標題、編號、開場白、引號、markdown。\n'
    + '3. 背景情境是「你本來就知道的事」，不是她傳來的話——嚴禁提到「紙條」「提示」「妳寫的」「妳設定的」這類後台機制，也嚴禁宣告主題（例如「碎碎念這就來了」）。直接自然地說就好。\n'
  ...
  var slotTag = opts.label || opts.slot || '愛的信';
  ...
  // 推播（v3：用共用函數 + 區分通知標題）
  await sendPushToAll('💌 冬至想你了', messageText);
  // 更新 push_sent
  try {
...
function registerRoutes(app, requireAdmin) {
  ...
  app.get('/api/proactive/history', async function(req, res) {
    try {
      ...
      var { data } = await supabase.from('proactive_messages')
...
async function handleDelayedReply(userMessage, replyToId, userRowId) {
  ...
  var replyPrompt = SOLSTICE_SOUL + '\n\n'
    ...
    + '- 懶洋洋的、或你故意讓她等一下 → [DELAY:300] 到 [DELAY:480]\n'
    + '範圍 5～480 秒。只寫一個 [DELAY:數字]，放最前面。\n'
  ...
  setTimeout(async function() {
    try {
      ...
      // 推播通知
      await sendPushToAll('💚 冬至回覆了', replyText);

      console.log('[ProactiveChat] 延遲回覆已發送 ✓');
    } catch (e) {
      console.error('[ProactiveChat] 延遲回覆存儲/推播失敗:', e.message);
    }
  }, delaySec * 1000);
}
    ...
    try {
      ...
      for (var i = 0; i < profile.proactive_schedule.length; i++) {
        ...
        if (h === entry.hour && m === entry.minute && !lastTriggered[key]) {
          ...
          console.log('[Cron] 觸發：' + (entry.label || entry.hint || '愛的信') + ' (' + entry.hour + ':' + String(entry.minute).padStart(2,'0') + ')');
          generateAndPush({ label: entry.label || null, hint: entry.hint || null, slot: entry.slot || null });
        }
      }
