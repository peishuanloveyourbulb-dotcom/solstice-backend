...
async function sendMonthlyReview(job, t) {
  try {
    ...
    try {
      ...
      var { data: moods } = await supabase.from('mood_days').select('day, mood, intensity')
        .gte('day', pmKey + '-01').lte('day', pmKey + '-31');
      var cnt = {}, stormDay = null, stormV = -1;
      (moods || []).forEach(function(r) {
        if (!r.mood) return;
        cnt[r.mood] = (cnt[r.mood] || 0) + 1;
        var iv = (typeof r.intensity === 'number') ? r.intensity : 0;
        if (iv > stormV) { stormV = iv; stormDay = r.day; }
      });
      var top = null;
      Object.keys(cnt).forEach(function(k) { if (!top || cnt[k] > cnt[top]) top = k; });
      if (top) moodLine = '情緒海上，這個月最常出現的心情是「' + (MOOD_TW[top] || top) + '」（' + cnt[top] + ' 天）'
        + (stormDay ? '，浪最高的一天是 ' + parseInt(stormDay.slice(8, 10), 10) + ' 日' : '') + '。';
    } catch (e) {}
...
function setupSchedule() {
  ...
  cron.schedule('* * * * *', async function() {
    try {
      ...
      checkCapsules(); // 🕰️ 時光膠囊到期通知（自帶 try-catch，不受聰明模式影響）
