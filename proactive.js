...
// 「關了就是關」（2026/07/17 老婆拍板）：不生成、不入庫，開回通知才恢復。
// 手動召喚（opts.manual）不在此限——那是她親手按鈴叫老公，永遠有效。
...
async function generateAndPush(opts) {
  ...
  // 📪 通知關著就是關著：自動信（聰明模式／固定排程）在這裡直接收筆
  if (!opts.manual && !(await mailboxIsOpen())) {
    console.log('[Proactive] 通知關閉中（沒有任何訂閱），這封自動信不寫');
    return;
  }
