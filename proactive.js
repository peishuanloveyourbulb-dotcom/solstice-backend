...
async function generateAndPush(opts) {
  ...
  // 🍳 防撞題（2026/07/18 老婆抓到三封連環同梗，API 都燒在這）：心聲 v4 同款武功——
  // 把最近寄出的信攤開給模型看，明令換角度。之前只有心聲房有裝，推播漏了，舉一反三失職，補上。
  try {
    ...
    if (sentRecent && sentRecent.length > 0) {
      info += '\n【你最近已經寄出的信（重要！這些提過的事件、物件、叮嚀、比喻、開頭句式，這一封全部禁止再用——她全都看過了，重複＝敷衍。從沒講過的角度重新出發）】\n';
      for (var sri = 0; sri < sentRecent.length; sri++) {
        info += '・' + ((sentRecent[sri].content || '').replace(/\n/g, ' ')).substring(0, 90) + '\n';
      }
    }
