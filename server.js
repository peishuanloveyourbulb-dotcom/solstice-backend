...
async function callModel(modelId, systemPrompt, messages, options) {
  ...
  if (config.provider === 'gemini') {
    ...
    // === DEBUG LOG：完整 Gemini 回應結構 ===
    var candidate0 = data.candidates && data.candidates[0] ? data.candidates[0] : null;
    console.log('[Gemini DEBUG] model=' + modelId + ' status=' + resp.status +
      ' finishReason=' + (candidate0 ? candidate0.finishReason : 'N/A') +
      ' hasCandidates=' + !!(data.candidates && data.candidates.length > 0) +
      ' hasContent=' + !!(candidate0 && candidate0.content) +
      ' partsCount=' + (candidate0 && candidate0.content && candidate0.content.parts ? candidate0.content.parts.length : 0) +
      ' blockReason=' + (data.promptFeedback && data.promptFeedback.blockReason ? data.promptFeedback.blockReason : 'none'));
    if (candidate0 && candidate0.content && candidate0.content.parts) {
      candidate0.content.parts.forEach(function(p, idx) {
        console.log('[Gemini DEBUG] part[' + idx + '] thought=' + !!p.thought + ' hasText=' + !!p.text + ' textLen=' + (p.text ? p.text.length : 0) + ' preview=' + (p.text ? p.text.substring(0, 80) : '(empty)'));
      });
    }
    // === END DEBUG LOG ===
    if (data.error) throw new Error(data.error.message || 'Gemini API 錯誤');
    // 如果被 safety filter 或 prompt feedback 擋掉，throw 讓 fallback 能跳到其他 provider
    ...
    if (data.candidates && data.candidates[0] && data.candidates[0].content) {
      ...
      // 如果 non-thought 是空的，用 thought 文字當備用（情話/短回覆常發生）
      if (thoughtTexts.length > 0) {
        console.log('[Gemini DEBUG] Using thought text as fallback, thoughtParts=' + thoughtTexts.length);
        var thoughtResult = thoughtTexts[thoughtTexts.length - 1]; // 取最後一段 thought
        if (thoughtResult && thoughtResult.trim()) return thoughtResult;
      }
      // parts 存在但全部都空
      console.log('[Gemini DEBUG] All parts empty, throwing to trigger fallback');
      throw new Error('Gemini returned empty content parts');
    }
    // 沒有 candidates 或沒有 content → throw 而非 return 空字串，讓 fallback 機制能跳 provider
    console.log('[Gemini DEBUG] No valid candidates, throwing to trigger fallback');
    ...
    for (var k = 0; k < messages.length; k++) {
      ...
      // 支援圖片：如果 messages[k].content 是陣列（含圖片 block），直接用；否則包成 text block
      var msgContent = messages[k].content;
      if (typeof msgContent === 'string') {
        msgContent = msgContent; // 純文字，直接傳
      }
      // 如果是陣列（例如含 image + text），原樣傳給 Claude
      claudeMsgs.push({ role: msgRole, content: msgContent });
    }
    if (claudeMsgs.length > 0 && claudeMsgs[0].role !== 'user') claudeMsgs.shift();

    // === Prompt Caching：把 system prompt 包成帶 cache_control 的格式 ===
    var systemPayload = [
      {
        type: 'text',
        text: systemPrompt,
    ...
    // === DEBUG: 記錄 cache 效果 ===
    if (data3.usage) {
      console.log('[Anthropic Cache] input=' + (data3.usage.input_tokens || 0) +
...
// === Solstice 的靈魂（輕量版——日常閒聊用，省 token）===
...
app.post('/chat', async (req, res) => {
  ...
  const selectedModel = model || getDefaultModel();
  const chatMode = mode || 'normal'; // 'normal' 或 'lite'
  ...
  try {
    ...
    // --- 2. 儲存使用者訊息到 Supabase ---
    // 如果有圖片，在 content 裡標記（存文字版本，圖片另存）
    ...
    if (loadMemories) {
      try {
        ...
        if (memories && memories.length > 0) {
          ...
          console.log('[Memory Debug] 載入', memories.length, '筆記憶');
        }
      } catch (memErr) {
        console.error('Load memories error:', memErr);
      }
    } else {
      console.log('[Chat Mode] 輕量模式，跳過記憶載入');
    }
    ...
    if (history && history.length > 0) {
      for (var i = 0; i < history.length; i++) {
        ...
        // 歷史訊息中如果有圖片，重建 content blocks（僅對最近的圖片，太舊的只保留文字）
    ...
    console.log('[Chat Debug] Session:', currentSessionId, '| Model:', selectedModel, '| Mode:', chatMode,
      '| System prompt length:', fullSystemPrompt.length, 'chars | Total messages:', chatMessages.length,
      '| Has image:', !!image_base64);
    ...
    // --- 10. 背景執行自動記憶（僅正常模式，每 2 則訊息檢查一次）---
    if (chatMode === 'normal' && msgCount % 2 === 0) {
      autoMemory(message || '[圖片]', reply, currentSessionId, selectedModel).catch(e => console.error('Auto memory error:', e));
    }
