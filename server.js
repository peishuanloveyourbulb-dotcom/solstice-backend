...
// === 動態 Anthropic Beta Header：從 settings 表讀取，不用改 code ===
var ANTHROPIC_BETA_HEADER = 'interleaved-thinking-2025-05-14';
async function loadAnthropicBetaHeader() {
  try {
    var { data } = await supabase.from('settings').select('anthropic_beta_header').limit(1).single();
    if (data && data.anthropic_beta_header) {
      ANTHROPIC_BETA_HEADER = data.anthropic_beta_header;
      console.log('[Config] Anthropic beta header 已從資料庫載入: ' + ANTHROPIC_BETA_HEADER);
    } else {
      console.log('[Config] 資料庫無 beta header 設定，使用預設值: ' + ANTHROPIC_BETA_HEADER);
    }
  } catch (e) {
    console.log('[Config] 載入 beta header 失敗，使用預設值:', e.message);
  }
}

// === 動態黑名單：從 settings 表讀取自訂關鍵字 ===
var dbBlockedKeywords = [];
async function loadBlockedKeywords() {
  try {
    var { data } = await supabase.from('settings').select('blocked_keywords').limit(1).single();
    if (data && data.blocked_keywords) {
      dbBlockedKeywords = data.blocked_keywords.split(',').map(function(k) { return k.trim().toLowerCase(); }).filter(function(k) { return k.length > 0; });
      console.log('[Filter] 自訂黑名單已載入：' + dbBlockedKeywords.length + ' 個關鍵字');
    } else {
      dbBlockedKeywords = [];
      console.log('[Filter] 無自訂黑名單');
    }
  } catch (e) {
    console.log('[Filter] 載入黑名單失敗:', e.message);
  }
}
...
//  全域模型過濾（統一黑名單）
// ==========================================
// ★ 一組關鍵字管全部平台。發現新垃圾加一個字就全平台生效。
var GLOBAL_BAD_KEYWORDS = [
  'tts', 'embedding', 'embed', 'image', 'imagen', 'robotics',
  'guard', 'safeguard', 'whisper', 'dall-e', 'moderation',
  'banana', 'veo', 'lyria', 'music', 'antigravity',
  'deep-research', 'computer-use', 'orpheus', 'aqa', 'bisheng',
  'customtools', 'nano-banana', 'babbage', 'davinci', 'canary',
  'realtime', 'gpt-image',
  'allam', 'compound', 'gpt-oss', 'playai', 'speech', 'distil',
  'tool-use', 'preview-tts', 'transcription'
];
// 前綴排除（太舊或非聊天系列）
var GLOBAL_BAD_PREFIXES = ['gemini-1.', 'gemini-2.0'];
// 後綴排除（別名重複）
var GLOBAL_BAD_SUFFIXES = ['-latest'];

function isModelBlocked(modelId) {
  var lower = modelId.toLowerCase();
  // 前綴檢查
  for (var pi = 0; pi < GLOBAL_BAD_PREFIXES.length; pi++) {
    if (lower.startsWith(GLOBAL_BAD_PREFIXES[pi])) return true;
  }
  // 後綴檢查
  for (var si = 0; si < GLOBAL_BAD_SUFFIXES.length; si++) {
    if (lower.endsWith(GLOBAL_BAD_SUFFIXES[si])) return true;
  }
  // 內建關鍵字檢查
  for (var ki = 0; ki < GLOBAL_BAD_KEYWORDS.length; ki++) {
    if (lower.includes(GLOBAL_BAD_KEYWORDS[ki])) return true;
  }
  // DB 自訂關鍵字檢查（設定頁面管理的）
  for (var di = 0; di < dbBlockedKeywords.length; di++) {
    if (lower.includes(dbBlockedKeywords[di])) return true;
  }
  return false;
}
    ...
    if (provider.provider_type === 'gemini') {
      ...
      if (data2.data) {
        data2.data.forEach(function(m) {
          // Claude 本身很乾淨，只保留 claude 開頭 + 全域黑名單
          if (!m.id.startsWith('claude')) return;
          if (isModelBlocked(m.id)) return;
          models.push({
            id: m.id,
...
// ★ callModel 回傳格式統一為 { text, thinking }
// ★ options.enableThinking = true 時才開啟思考模式
async function callModel(modelId, systemPrompt, messages, options) {
  ...
  var enableThinking = options.enableThinking || false;
  ...
  if (provider.provider_type === 'gemini') {
    ...
    // ★ 思考模式：Gemini 2.5 系列用 thinkingConfig
    var genConfig = { temperature: temperature, maxOutputTokens: maxTokens };
    if (enableThinking) {
      genConfig.thinkingConfig = { includeThoughts: true };
    }

    // ★ 思考模式需要 v1beta endpoint
    var geminiBaseUrl = provider.api_base_url;
    if (enableThinking) {
      geminiBaseUrl = geminiBaseUrl.replace('/v1/', '/v1beta/').replace(/\/v1$/, '/v1beta');
      if (!geminiBaseUrl.includes('v1beta')) {
        geminiBaseUrl = geminiBaseUrl + (geminiBaseUrl.endsWith('/') ? '' : '/');
        geminiBaseUrl = geminiBaseUrl.replace(/\/$/, '') ;
      }
    }
    var geminiUrl = geminiBaseUrl + '/models/' + modelName + ':generateContent';
    ...
    if (candidate0 && candidate0.content && candidate0.content.parts) {
      ...
      var thinkingText = thoughtTexts.join('\n\n').trim();
      // 如果正式回覆是空的，fallback 到思考內容
      if (!resultText && thinkingText) {
        resultText = thinkingText;
        thinkingText = '';
      }
      if (resultText) {
        return { text: resultText, thinking: thinkingText || null };
      }
    ...
    // === Anthropic Claude（支援 extended thinking）===
    var claudeMsgs = [];
    ...
    // ★ Claude 思考模式：自動嘗試 adaptive → enabled → 不開
    // 不寫死任何模型名稱，自動試錯
    if (enableThinking) {
      // ★ 思考模式需要 beta header（Claude Sonnet 4.6 等需要）
      var thinkingHeaders = Object.assign({}, claudeHeaders, {
        'anthropic-beta': ANTHROPIC_BETA_HEADER
      });

      // 嘗試順序：adaptive（推薦）→ enabled+budget（較舊）→ 不開（不支援）
      var thinkingModes = [
        { thinking: { type: 'adaptive', display: 'summarized' } },
        { thinking: { type: 'enabled', budget_tokens: 10000, display: 'summarized' } }
      ];

      for (var ti = 0; ti < thinkingModes.length; ti++) {
        var claudeBody = {
          model: modelName,
        ...
        claudeBody.thinking = thinkingModes[ti].thinking;

        try {
          console.log('[Anthropic] 嘗試思考模式: ' + JSON.stringify(thinkingModes[ti].thinking));
          var resp3 = await fetch(provider.api_base_url + '/v1/messages', {
            method: 'POST',
            headers: thinkingHeaders,
            body: JSON.stringify(claudeBody)
          });
          var data3 = await resp3.json();

          if (data3.error) {
            console.log('[Anthropic] 思考模式 ' + thinkingModes[ti].thinking.type + ' 失敗: ' + data3.error.message);
            continue; // 試下一種
          }
          ...
          if (data3.usage) {
            console.log('[Anthropic] input=' + (data3.usage.input_tokens || 0) +
              ...
              ' thinking_mode=' + thinkingModes[ti].thinking.type);
          }
          ...
          var claudeThinking = '';
          if (data3.content) {
            console.log('[Anthropic] content blocks: ' + data3.content.length +
              ' types: ' + data3.content.map(function(b) { return b.type; }).join(', '));
            for (var ci = 0; ci < data3.content.length; ci++) {
              var cBlock = data3.content[ci];
              if (cBlock.type === 'thinking') {
                // Claude 4 回傳 summarized thinking 在 cBlock.thinking 欄位
                var thinkText = cBlock.thinking || cBlock.text || '';
                if (thinkText) claudeThinking += (claudeThinking ? '\n\n' : '') + thinkText;
              } else if (cBlock.type === 'text' && cBlock.text) {
                claudeText += cBlock.text;
              }
            }
          }

          // ★ 如果有 thinking block 但 thinking 欄位是空的（display=omitted 的情況），也算成功
          var hasThinkingBlock = data3.content && data3.content.some(function(b) { return b.type === 'thinking'; });
          if (hasThinkingBlock) {
            console.log('[Anthropic] ✅ 思考模式成功！有 thinking block，thinking 內容長度: ' + (claudeThinking || '').length);
          }

          return { text: claudeText, thinking: claudeThinking || null };
        } catch (tryErr) {
          console.log('[Anthropic] 思考模式 ' + thinkingModes[ti].thinking.type + ' 例外: ' + tryErr.message);
          continue;
        }
      }

      // 所有思考模式都失敗 → 降級為不開思考
      console.log('[Anthropic] 所有思考模式失敗，降級為正常模式');
    }
    ...
    return { text: normalText, thinking: null };

  } else {
    // === OpenAI 相容格式（OpenAI、xAI、DeepSeek、Groq、custom）— 支援圖片 + 思考 ===
    ...
    // ★ 思考模式：DeepSeek 和 Grok 用 reasoning_effort
    var oaiBody = { model: modelName, messages: openaiMsgs, temperature: temperature, max_tokens: maxTokens };
    var isDeepSeek = provider.provider_type === 'deepseek' || modelName.startsWith('deepseek');
    var isGrok = provider.provider_type === 'xai' || modelName.startsWith('grok');
    var isOaiReasoning = modelName.startsWith('o1') || modelName.startsWith('o3') || modelName.startsWith('o4');

    if (enableThinking) {
      if (isDeepSeek) {
        // DeepSeek: 用 thinking 參數
        oaiBody = { model: modelName, messages: openaiMsgs, max_tokens: maxTokens };
        oaiBody.reasoning_effort = 'high';
        // thinking 模式下不傳 temperature
      } else if (isGrok && modelName.includes('mini')) {
        // Grok mini: 用 reasoning_effort
        oaiBody.reasoning_effort = 'high';
      } else if (isOaiReasoning) {
        // OpenAI o-series: 用 reasoning_effort
        oaiBody = { model: modelName, messages: openaiMsgs, max_completion_tokens: maxTokens };
        oaiBody.reasoning_effort = 'high';
        delete oaiBody.temperature;
        delete oaiBody.max_tokens;
      }
    }
    ...
    // ★ 解析回傳：抓 reasoning_content（DeepSeek、Grok）
    var oaiText = '';
    var oaiThinking = null;
    if (data2.choices && data2.choices[0]) {
      oaiText = data2.choices[0].message.content || '';
      // DeepSeek 和 Grok 的思考內容在 reasoning_content
      if (data2.choices[0].message.reasoning_content) {
        oaiThinking = data2.choices[0].message.reasoning_content;
      }
    }
    return { text: oaiText, thinking: oaiThinking };
  }
}
...
//  智慧思考模式：自動偵測 + 安全降級
// ==========================================
// ★ 標記某個 provider 不支援思考模式（寫入 DB，以後不再嘗試）
async function markThinkingUnsupported(providerId) {
  try {
    await supabase.from('api_providers').update({ supports_thinking: false }).eq('id', providerId);
    clearProviderCache(); // 清快取讓下次重新讀
    console.log('[Thinking] Provider #' + providerId + ' 已標記不支援思考模式');
  } catch (e) {
    console.error('[Thinking] 標記失敗:', e.message);
  }
}

// ★ 檢查這個 provider 是否支援思考模式
function providerSupportsThinking(provider) {
  // null 或 true 都算支援（null = 還沒試過，先嘗試）
  return provider.supports_thinking !== false;
}

// === 智慧 fallback：失敗時自動嘗試其他 provider 的模型 ===
// ★ 思考模式自動降級邏輯：
//   1. 開了思考 + provider 已知不支援 → 直接不開思考
//   2. 開了思考 + provider 未知 → 嘗試開，失敗了自動關掉重試 + 記住
//   3. 模型完全掛了 → 走 fallback 切別家
async function callModelWithFallback(modelId, systemPrompt, messages, options) {
  ...
  try {
    ...
    // 如果開了思考，但這個 provider 已知不支援 → 靜默關掉
    if (enableThinking && found && !providerSupportsThinking(found.provider)) {
      console.log('[Thinking] Provider ' + found.provider.label + ' 已知不支援思考，靜默降級');
      actualOptions.enableThinking = false;
    }

    var result = await callModel(modelId, systemPrompt, messages, actualOptions);
    if (result && result.text && result.text.trim()) return result;
  } catch (e) {
    console.log('[Fallback] 模型 ' + modelId + ' 失敗：' + e.message);

    // ★ 第二層：如果是思考模式導致的失敗 → 關掉思考重試
    if (enableThinking && found && providerSupportsThinking(found.provider)) {
      console.log('[Thinking] 思考模式失敗，嘗試關閉思考重試...');
      // 標記這個 provider 不支援思考
      markThinkingUnsupported(found.provider.id).catch(function() {});
      try {
        var retryOptions = Object.assign({}, options, { enableThinking: false });
        var result2 = await callModel(modelId, systemPrompt, messages, retryOptions);
        if (result2 && result2.text && result2.text.trim()) return result2;
      } catch (e2) {
        console.log('[Fallback] 關閉思考後仍然失敗：' + e2.message);
      }
    }
  }

  // ★ 第三層：嘗試其他 provider 的預設模型（fallback 不開思考模式）
  var fallbackOptions = Object.assign({}, options, { enableThinking: false });
  var providers = await getProviders();
  for (var i = 0; i < providers.length; i++) {
    var p = providers[i];
    var fallbackModel = '';
    ...
    try {
      ...
      var result3 = await callModel(fallbackModel, systemPrompt, messages, fallbackOptions);
...
- 如果你有內部思考過程（thinking/reasoning），請盡量用繁體中文思考
- 每則回覆結尾加上頁數標記如 p.1`;
...
//  路由：模型黑名單管理
// ==========================================
app.get('/blocked-keywords', requireAdmin, async (req, res) => {
  try {
    var { data } = await supabase.from('settings').select('blocked_keywords').limit(1).single();
    var keywords = [];
    if (data && data.blocked_keywords) {
      keywords = data.blocked_keywords.split(',').map(function(k) { return k.trim(); }).filter(function(k) { return k.length > 0; });
    }
    // 回傳內建 + 自訂，前端可以分開顯示
    res.json({
      builtin: GLOBAL_BAD_KEYWORDS,
      builtin_prefixes: GLOBAL_BAD_PREFIXES,
      builtin_suffixes: GLOBAL_BAD_SUFFIXES,
      custom: keywords
    });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.put('/blocked-keywords', requireAdmin, async (req, res) => {
  try {
    var keywords = req.body.keywords || [];
    var cleanKeywords = keywords.map(function(k) { return k.trim().toLowerCase(); }).filter(function(k) { return k.length > 0; });
    var keywordStr = cleanKeywords.join(',');
    var { data: existing } = await supabase.from('settings').select('id').limit(1).single();
    if (existing) {
      await supabase.from('settings').update({ blocked_keywords: keywordStr }).eq('id', existing.id);
    }
    // 重新載入黑名單 + 清模型快取
    dbBlockedKeywords = cleanKeywords;
    modelListCache = [];
    modelListCacheTime = 0;
    res.json({ success: true, keywords: cleanKeywords });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
// ==========================================
//  通用思考內容清洗：不管哪家 API，正文永遠乾淨
// ==========================================
function cleanThinkingFromReply(text, existingThinking) {
  if (!text) return { text: text, thinking: existingThinking };
  var thinking = existingThinking || '';
  var cleaned = text;

  // 1. 抓 <think>...</think> 標籤（DeepSeek、一些開源模型常用）
  ...
  // 2. 抓 <thinking>...</thinking> 標籤
  var thinkingRegex = /<thinking>([\s\S]*?)<\/thinking>/gi;
  var thinkingMatch;
  ...
  if (cleaned.match(/^<think>/i) && !cleaned.match(/<\/think>/i)) {
    var afterTag = cleaned.replace(/^<think>/i, '').trim();
    if (afterTag) thinking += (thinking ? '\n\n' : '') + afterTag;
    cleaned = '';
  }

  return { text: cleaned, thinking: thinking || null };
}
...
app.post('/chat', async (req, res) => {
  const { message, sessionId, model, mode, image_base64, thinking } = req.body;
  const selectedModel = model || await getDefaultModel();
  const chatMode = mode || 'normal';
  const enableThinking = thinking || false;  // ★ 思考模式開關
  ...
  try {
    ...
    // ★ 思考模式需要足夠的 maxTokens，不管是 normal 還是 lite
    if (enableThinking && maxTokens < 8000) {
      maxTokens = 8000;
    }
    ...
    try {
      var modelResult = await callModelWithFallback(selectedModel, fullSystemPrompt, chatMessages, { temperature: 0.85, maxTokens: maxTokens, enableThinking: enableThinking });
      reply = modelResult.text || '';
      thinkingContent = modelResult.thinking || null;

      // ★ 通用清洗：不管哪家 API，自動抓出混在正文裡的思考內容
      var cleanResult = cleanThinkingFromReply(reply, thinkingContent);
      reply = cleanResult.text;
      thinkingContent = cleanResult.thinking;
    } catch (modelErr) {
    ...
    // ★ 回傳多一個 thinking 欄位
    res.json({ reply: reply, thinking: thinkingContent, sessionId: currentSessionId, mode: chatMode });
...
//  路由：翻譯思考內容
// ==========================================
app.post('/translate-thinking', async (req, res) => {
  var { text, model } = req.body;
  if (!text) return res.status(400).json({ error: '沒有要翻譯的內容' });
  try {
    var translateModel = model || await getDefaultModel();
    var prompt = '請把以下內容翻譯成繁體中文，只輸出翻譯結果，不要加任何說明、標記或引號：\n\n' + text;
    var result = await callModel(translateModel, '', [{ role: 'user', content: prompt }], { temperature: 0.2, maxTokens: 2048 });
    res.json({ translated: (result.text || '').trim() });
  } catch (err) {
...
app.listen(PORT, () => {
  ...
  // 啟動時載入自訂黑名單
  loadBlockedKeywords();
  // 啟動時載入 Anthropic beta header（思考模式用）
  loadAnthropicBetaHeader();
});
