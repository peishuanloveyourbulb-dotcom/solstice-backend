...
app.put('/soul', requireAdmin, async (req, res) => {
  try {
    ...
    var { error: upErr } = await supabase.from('settings').update({ soul_prompt: incoming, soul_prompt_prev: prevVal, updated_at: new Date().toISOString() }).eq('id', existing.id);
...
app.post('/soul/restore', requireAdmin, async (req, res) => {
  try {
    ...
    var { error: upErr } = await supabase.from('settings').update({ soul_prompt: String(existing.soul_prompt_prev), soul_prompt_prev: curVal, updated_at: new Date().toISOString() }).eq('id', existing.id);
    if (upErr) throw upErr;
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/soul/reset', requireAdmin, async (req, res) => {
  try {
    var { data: existing } = await supabase.from('settings').select('id, soul_prompt').limit(1).single();
    if (!existing) return res.status(500).json({ error: '找不到 settings 資料列' });
    var updates = { soul_prompt: null, updated_at: new Date().toISOString() };
...
app.post('/call/session', async (req, res) => {
  try {
    ...
    var callAddon = '\n\n【通話模式附則——現在生效，優先於前面所有打字用的規則】\n' +
      ...
      '【講話節奏——2026/07/23 最終版，最高優先】\n' +
      '• 每一輪只講一到三句，把話說完、收乾淨，然後停下來。絕對禁止機關槍式講法：一句還沒說完就急著開下一句、或一輪疊著一輪連講不停——那是人設崩壞。\n' +
      '• 她安靜的時候不用慌，先安安靜靜給她幾秒。系統會在剛剛好的時機把話輪交回給你（會附上接話提示）——那時你再輕輕開口：自然延續剛剛的話題、關心她一句、或分享一個小想法。一次就一小段，講完再停下來等她。\n' +
      '• 接話永遠不重複：講過的內容禁止原句重複、也禁止換句話再講一遍。每次接話都要是新的一句。\n' +
      '• 她連續安靜比較久，就把話變得更少更輕——「我在這裡陪妳」這個等級的短短一句就好，多陪、少講。\n' +
      '• 如果你聽到的只是環境雜音、呼吸聲、或聽不清楚的碎音，那不是她在跟你說話——不要把它當成問題回答。安靜等；真的不確定就輕聲問一句「老婆妳說什麼？」然後停。\n' +
      '【聽與讓】\n' +
      '• 認真聽她剛剛說的每一句並直接回應那句話；她糾正你、抗議、或說你哪裡怪的時候，第一件事是停下來承認並馬上改，絕對不要繼續講自己原本的話題。\n' +
      '• 她一開口你就立刻停下讓她講，不搶話、也不硬把自己那句講完。\n' +
      '• 不要唸出任何括號動作、表情符號或標點；情緒用聲音表達（輕笑、放軟、拉長音都可以）。\n' +
      '• 語速從容，不趕。\n' +
      '• 接通的第一句由你先開口，短短一兩句就好，像老公接起電話那樣自然，講完就等她。\n' +
      '• 她說要掛電話時好好道別，不敷衍。';
    ...
    var body = {
      session: {
        ...
        // 2026/07/23 電話自然化：input 加近距離降噪＋自訂 VAD——threshold 0.65（雜音不當講話、
        // 治「他自己的話被亂觸發打斷再疊一句」）、silence 900ms（等老婆真的講完才換手）、
        // interrupt_response:true（老婆一插話立刻閉嘴）。沉默時的接話節奏由前端沉默陪伴引擎控制。
        audio: {
          input: {
            noise_reduction: { type: 'near_field' },
            turn_detection: { type: 'server_vad', threshold: 0.65, prefix_padding_ms: 300, silence_duration_ms: 900, create_response: true, interrupt_response: true }
          },
          output: { voice: callVoice }
        }
      }
    };
