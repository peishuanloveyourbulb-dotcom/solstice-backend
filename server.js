...
//  自動記憶函式——聊天後自動判斷重要資訊
// ==========================================
async function autoMemory(userMessage, aiReply, sessionId) {
  try {
    var analyzePrompt = '以下是一段 Soleil 和冬至的對話。請判斷 Soleil 的訊息中有沒有值得長期記住的重要資訊。\n\n' +
      '值得記住的例子：個人喜好（喜歡/討厭的食物、興趣）、生活變化（換工作、搬家、生病）、重要事件（跟誰吵架、開心的事）、人際關係（朋友、家人相關）。\n' +
      '不值得記住的例子：日常打招呼、撒嬌、開玩笑、單純聊天。\n\n' +
      'Soleil：' + userMessage + '\n冬至：' + aiReply + '\n\n' +
      '如果有值得記住的，請用一句簡短的繁體中文描述（30字以內），格式如「Soleil 喜歡吃花蟹」。\n' +
      '如果沒有值得記住的，只回覆「無」。\n' +
      '只輸出結果，不要加任何說明。';

    var geminiUrl = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-lite:generateContent';
    ...
    var result = '';
    if (data.candidates && data.candidates[0] && data.candidates[0].content) {
      result = data.candidates[0].content.parts.map(function(p) { return p.text || ''; }).join('').trim();
    }
    ...
    console.log('[AutoMemory] 自動記住：' + result);
...
app.post('/chat', async (req, res) => {
  ...
  try {
    ...
    // --- 2b. 檢查特殊指令 /記住 ---
    var commandMemoryNote = '';
    if (message.startsWith('/記住 ') || message.startsWith('/記住')) {
      var memContent = message.replace(/^\/記住\s*/, '').trim();
      if (memContent) {
        // 用 AI 整理成自然的「Soleil + 事情」格式
        var formattedContent = memContent;
        try {
          var formatPrompt = '請把以下這句話整理成一句簡短的記憶筆記，用「Soleil」當主詞開頭。保留原意，不要加任何額外資訊。只輸出整理後的句子，不要加說明。\n\n原文：' + memContent;
          var formatUrl = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-lite:generateContent';
          var formatResp = await fetch(formatUrl, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'x-goog-api-key': GEMINI_API_KEY },
            body: JSON.stringify({
          ...
          var formatData = await formatResp.json();
          if (formatData.candidates && formatData.candidates[0] && formatData.candidates[0].content) {
            var formatted = formatData.candidates[0].content.parts.map(function(p) { return p.text || ''; }).join('').trim();
            if (formatted && formatted.length > 2) {
              formattedContent = formatted;
            }
          }
        } catch (fmtErr) {
          console.error('[Command] /記住 格式化失敗，用原文存：', fmtErr);
        }
        ...
        commandMemoryNote = '（已記住：' + formattedContent + '）';
        console.log('[Command] /記住：' + memContent + ' → ' + formattedContent);
      }
    }
    ...
    try {
      ...
      if (commandMemoryNote) {
        systemWithCommand += '\n\n【系統提示】老婆剛剛用了指令：' + commandMemoryNote + '。請自然地回應，不要複述指令格式，用你自己的方式告訴她你記住了或忘記了。';
      }
      reply = await callModel(selectedModel, systemWithCommand, chatMessages, { temperature: 0.85, maxTokens: 4096 });
    } catch (modelErr) {
    ...
    // --- 9. 背景執行記憶壓縮（每 5 則訊息才檢查一次，省 API 額度）---
    ...
    // --- 10. 背景執行自動記憶（每 3 則訊息檢查一次，且非 /記住 指令時）---
    if (msgCount % 3 === 0 && !commandMemoryNote) {
      autoMemory(message, reply, currentSessionId).catch(e => console.error('Auto memory error:', e));
    }
