...
// ==========================================
//  🔊 TTS 語音合成（OpenAI TTS API）
// ==========================================
app.post('/api/tts', async (req, res) => {
  try {
    var { text } = req.body;
    if (!text || !text.trim()) {
      return res.status(400).json({ error: '沒有文字內容' });
    }

    var apiKey = process.env.OPENAI_API_KEY;
    if (!apiKey) {
      return res.status(503).json({ error: 'TTS 服務未啟用（無 OpenAI API Key）', fallback: true });
    }

    // 清理文字：移除星號動作描述，只留對話文字
    var cleanText = text
      .replace(/\*[^*]+\*/g, '')          // 移除 *動作描述*
      .replace(/\n{2,}/g, '\n')           // 多餘換行合併
      .replace(/^\s+|\s+$/g, '')          // 頭尾空白
    ...
    if (!cleanText) {
      return res.status(400).json({ error: '清理後沒有可朗讀的文字' });
    }
    ...
    var ttsResp = await fetch('https://api.openai.com/v1/audio/speech', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    if (!ttsResp.ok) {
      var errBody = '';
      try { errBody = await ttsResp.text(); } catch(e) {}
      console.error('[TTS] OpenAI API error:', ttsResp.status, errBody);
      return res.status(ttsResp.status).json({ error: 'TTS API 錯誤', fallback: true });
    }
    ...
    var arrayBuf = await ttsResp.arrayBuffer();
    res.send(Buffer.from(arrayBuf));

    console.log('[TTS] 成功生成語音，文字長度:', cleanText.length);

  } catch (err) {
    console.error('[TTS] Error:', err);
    res.status(500).json({ error: 'TTS 生成失敗：' + err.message, fallback: true });
  }
});
