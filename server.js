...
async function callModel(modelId, systemPrompt, messages, options) {
  ...
  if (provider.provider_type === 'openai') {
    ...
    if (data.error) {
      ...
      if (useThinking && (errMsg.indexOf('thinking') !== -1 || errMsg.indexOf('not supported') !== -1 || errMsg.indexOf('adaptive') !== -1)) {
        console.log('[auto-retry] ' + modelName + ' 不支援 thinking，自動移除並重試');
