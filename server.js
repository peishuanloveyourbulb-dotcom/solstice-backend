...
async function callModel(modelId, systemPrompt, messages, options) {
  ...
  // 安全限制：某些模型不支援超過 1.0 的 temperature，自動降溫
  if (temperature > 1.0) {
    console.log('[callModel] Clamping temperature from ' + temperature + ' to 1.0 for model ' + modelId);
    temperature = 1.0;
  }
...
app.post('/capsules/generate', async (req, res) => {
  ...
  try {
    ...
    var loveMsg = '';
    try {
      loveMsg = await callModelWithFallback(selectedModel, SOLSTICE_SOUL, [{ role: 'user', content: capsulePrompt }], { temperature: 0.95, maxTokens: 2048 });
    } catch (genErr) {
...
app.post('/daily-question/generate', async (req, res) => {
  ...
  try {
    ...
    var question = '';
    try {
      question = await callModelWithFallback(selectedModel, SOLSTICE_SOUL, [{ role: 'user', content: questionPrompt }], { temperature: 0.95, maxTokens: 2048 });
    } catch (genErr) {
