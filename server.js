...
app.post('/heartbeat/generate', async (req, res) => {
  try {
    ...
    //
    // 【對話素材的抓法 — 深入版跨聊天室】
    // 先撈最近 200 則訊息找出「最近有活動的聊天室清單」（用 session_id 去重）
    // 再對每個 session 各拉一段最近的訊息，這樣即使老婆最近一整週都窩在同一個聊天室，
    // 池子裡也一定會有多個房間的內容可以抽樣，不會被單一房間灌爆。
    ...
    // 找出最近有活動的 session_id 們（按活動新舊排序，保留前 6 個房間）
    var seenSessions = [];
    var seenSet = {};
    for (var ri = 0; ri < recentMsgsRaw.length; ri++) {
      var sid = String(recentMsgsRaw[ri].session_id || 0);
      if (!seenSet[sid]) {
        seenSet[sid] = true;
        seenSessions.push(sid);
        if (seenSessions.length >= 6) break;
      }
    }

    // 對每個活躍 session 各撈最近 20 則訊息（會蓋掉最近 200 則沒抓到的舊房間內容）
    // 只在有多個房間時才發額外請求，單一房間直接用原本的 recentMsgsRaw
    var allMsgs = recentMsgsRaw;
    if (seenSessions.length > 1) {
