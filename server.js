...
var modelQuirksCacheLoaded = false;

async function loadModelQuirks() {
  if (modelQuirksCacheLoaded) return modelQuirksCache;
  try {
    ...
    if (error) {
      ...
      modelQuirksCacheLoaded = true;
      return modelQuirksCache;
    }
    if (data) {
      for (var qi = 0; qi < data.length; qi++) {
        modelQuirksCache[data[qi].model_name] = data[qi].quirks || {};
      }
    }
    modelQuirksCacheLoaded = true;
...
async function callModel(modelId, systemPrompt, messages, options) {
  ...
  if (provider.provider_type === 'openai') {
    ...
    if (useThinking) {
      // adaptive thinking：不送 temperature，max_tokens 至少 16000
      claudeBody.thinking = { type: 'adaptive' };
      if (claudeBody.max_tokens < 16000) claudeBody.max_tokens = 16000;
      // thinking 模式不能送 temperature
    } else {
      // 正常模式：動態判斷 temperature
    ...
    console.log('[Anthropic] 送出 → model:', modelName, '| thinking:', useThinking ? 'adaptive' : 'off', '| temperature:', claudeBody.temperature || 'none', '| max_tokens:', claudeBody.max_tokens);
    var data = await resp.json();

    // 錯誤處理：thinking 或 temperature 不支援時自動重試
    if (data.error) {
      var errMsg = (data.error.message || '').toLowerCase();

      // thinking 不支援 → 記住 quirk，拿掉 thinking 改回正常模式重試
      var isThinkingError = useThinking && (
        (errMsg.indexOf('thinking') !== -1) ||
        (errMsg.indexOf('adaptive') !== -1) ||
        (errMsg.indexOf('budget_tokens') !== -1)
      );
      if (isThinkingError) {
        console.log('[auto-retry] ' + modelName + ' 不支援 thinking，錯誤：' + errMsg + '，自動移除並重試');
        await recordModelQuirk(modelName, 'no_thinking');
        delete claudeBody.thinking;
        // 恢復正常模式的 temperature 和 max_tokens
        if (!hasQuirk(modelName, 'no_temperature')) {
          claudeBody.temperature = temperature;
        }
      ...
      // temperature 不支援 → 記住 quirk，拿掉 temperature 重試
    ...
    if (data.content) {
      for (var ni = 0; ni < data.content.length; ni++) {
        if (data.content[ni].type === 'thinking' && data.content[ni].thinking) {
          thinkingText += data.content[ni].thinking;
