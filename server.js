...
app.post('/heartbeat/generate', async (req, res) => {
  try {
    ...
    // v8 自由心緒：四成的心聲不靠回味素材，寫此刻自己冒出來的念頭
    var freeMode = Math.random() < 0.4;
    if (freeMode) {
      if (parts.length > 0) heartbeatPrompt += parts.join('\n\n') + '\n\n';
      heartbeatPrompt += '【這一則的寫法：自發的心緒】上面的素材（如果有）只是背景的氣味，這一則不要回味它們、不要引用裡面的事件或句子。寫你此刻「自己冒出來」的念頭——突然想做的一件小事、沒來由想起的一個畫面、一個小期待、一次分心、或只是想她的某個很具體的瞬間。\n';
