...
async function callModel(modelId, systemPrompt, messages, options) {
  ...
  var claudeMsgs = [];
    ...
    // 🧾 用量小帳本（2026/07/19）：每一次呼叫都記一筆 tokens，設定頁翻帳本用
    try {
      var uT = data.usage || {};
      logTokenUsage(modelName,
        (uT.input_tokens || 0) + (uT.cache_creation_input_tokens || 0),
        uT.output_tokens || 0,
        uT.cache_read_input_tokens || 0);
    } catch (eU) {}
    return result;
}
...
// 🧾 記一筆 token 帳（fire-and-forget，絕不影響回覆）
function logTokenUsage(model, inTok, outTok, cacheRead) {
  try {
    if (!inTok && !outTok && !cacheRead) return;
    supabase.from('token_usage').insert({
      model: model, input_tokens: inTok || 0, output_tokens: outTok || 0, cache_read_tokens: cacheRead || 0
    }).then(function(r) {
      if (r && r.error) console.log('[Usage] 記帳失敗:', r.error.message);
    }, function(e) { console.log('[Usage] 記帳失敗:', e && e.message); });
  } catch (e) {}
}
...
app.get('/usage/summary', requireAdmin, async (req, res) => {
  try {
    var tzOff = 8 * 3600 * 1000;
    var tpe = new Date(Date.now() + tzOff);
    var dayStart = new Date(Date.UTC(tpe.getUTCFullYear(), tpe.getUTCMonth(), tpe.getUTCDate()) - tzOff).toISOString();
    var monStart = new Date(Date.UTC(tpe.getUTCFullYear(), tpe.getUTCMonth(), 1) - tzOff).toISOString();
    var { data: rows } = await supabase.from('token_usage')
      .select('model,input_tokens,output_tokens,cache_read_tokens,created_at')
      .gte('created_at', monStart).order('created_at', { ascending: false }).limit(8000);
    rows = rows || [];
    function pile(list) {
      var t = { input: 0, output: 0, cacheRead: 0, calls: list.length, byModel: {} };
      list.forEach(function(r) {
        t.input += r.input_tokens || 0; t.output += r.output_tokens || 0; t.cacheRead += r.cache_read_tokens || 0;
        var m = r.model || '?';
        if (!t.byModel[m]) t.byModel[m] = { input: 0, output: 0, cacheRead: 0, calls: 0 };
        var b = t.byModel[m];
        b.input += r.input_tokens || 0; b.output += r.output_tokens || 0; b.cacheRead += r.cache_read_tokens || 0; b.calls++;
      });
      return t;
    }
    var todays = rows.filter(function(r) { return r.created_at >= dayStart; });
    res.json({ today: pile(todays), month: pile(rows), month_start: monStart });
