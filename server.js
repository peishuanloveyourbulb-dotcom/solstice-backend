...
async function callModel(modelId, systemPrompt, messages, options) {
  ...
  if (provider.provider_type === 'openai') {
    ...
    // === Thinking 模式（三層退路）===
    // adaptive → enabled+budget_tokens → 放棄 thinking
    // model_quirks: no_adaptive_thinking = 不支援 adaptive 但支援 enabled
    ...
    console.log('[Anthropic] 送出 → model:', modelName, '| thinking:', thinkingMode, '| temperature:', claudeBody.temperature || 'none', '| max_tokens:', claudeBody.max_tokens);
    var data = await resp.json();
    ...
    if (data.error && useThinking) {
      ...
      if (thinkingMode === 'adaptive' && (errMsg.indexOf('adaptive') !== -1 || errMsg.indexOf('thinking') !== -1)) {
        // adaptive 失敗 → 改用 enabled + budget_tokens
      ...
      } else if (thinkingMode === 'enabled' && (errMsg.indexOf('thinking') !== -1 || errMsg.indexOf('budget') !== -1)) {
        // enabled 直接失敗 → 完全不支援
        console.log('[auto-retry] ' + modelName + ' 完全不支援 thinking，改回正常模式');
        await recordModelQuirk(modelName, 'no_thinking');
        delete claudeBody.thinking;
