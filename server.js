...
app.patch('/sessions/:id', async (req, res) => {
  try {
    ...
    // 使用 rpc 呼叫 SQL function，繞過 Supabase JS client 的 .update() 問題
    var params = { p_id: parseInt(req.params.id, 10) };
    if (name !== undefined) { params.p_name = name; }
    if (pinned !== undefined) { params.p_pinned = pinned; }
    if (color !== undefined) { params.p_color = color; }
    if (emoji !== undefined) { params.p_emoji = emoji; }
    const { data, error } = await supabase.rpc('update_session', params);
    if (error) throw error;
    res.json(data);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
