...
app.post('/capsules/generate', async (req, res) => {
  ...
  try {
    ...
    var loveMsg = '';
    try {
      loveMsg = await callModel(selectedModel, SOLSTICE_SOUL, [{ role: 'user', content: capsulePrompt }], { temperature: 1.2, maxTokens: 500 });
    } catch (genErr) {
    ...
    if (loveMsg.length > 500) loveMsg = loveMsg.substring(0, 500);
