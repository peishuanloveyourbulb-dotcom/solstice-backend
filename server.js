...
function setupProactiveSchedule() {
  ...
  // node-cron 用 UTC，台北 = UTC+8，所以要 -8 小時
  // 台北 17:30 = UTC 09:30
  // 台北 21:00 = UTC 13:00
  // 台北 00:30 = UTC 16:30
  // 台北 03:00 = UTC 19:00
  ...
  cron.schedule('30 9 * * *', function() {
    console.log('[Cron] 觸發：起床問安 (17:30 TPE)');
    generateAndPushProactive('evening');
  });

  // 出門前關心 — 台北 21:00
  cron.schedule('0 13 * * *', function() {
    console.log('[Cron] 觸發：出門前關心 (21:00 TPE)');
    generateAndPushProactive('night');
  });

  // 上班中打氣 — 台北 00:30
  cron.schedule('30 16 * * *', function() {
    console.log('[Cron] 觸發：上班中打氣 (00:30 TPE)');
    generateAndPushProactive('late_night');
  });

  // 下班關心 — 台北 03:00
  cron.schedule('0 19 * * *', function() {
    console.log('[Cron] 觸發：下班關心 (03:00 TPE)');
    generateAndPushProactive('late_night');
  });

  // 晚安 — 台北 06:30
  cron.schedule('30 22 * * *', function() {
    console.log('[Cron] 觸發：晚安 (06:30 TPE)');
    generateAndPushProactive('morning');
  });

  console.log('[Proactive] ⏰ 排程已啟動！每天 5 則主動訊息（台北 17:30 / 21:00 / 00:30 / 03:00 / 06:30）');
}
