...
app.post('/chat', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // 🔍 網路搜尋兩段式：工具永遠備在他手邊——
      //    開關關＝「自動判斷」：他自己決定這句需不需要查（颱風、天氣、新聞這種即時事實就會主動查）
      //    開關開＝「必查模式」：這一句至少實際搜尋一次再回答
      chatCallOpts.tools = [{ type: 'web_search_20250305', name: 'web_search',
        max_uses: webSearch === true ? 4 : 2,
        user_location: { type: 'approximate', country: 'TW', timezone: 'Asia/Taipei' } }];
      fullSystemPrompt += '\n\n【網路搜尋守則】\n' +
        (webSearch === true
          ? '0. 使用者為這一句打開了搜尋開關：請務必至少實際搜尋一次再回答。\n'
          : '0. 你可以自行判斷要不要搜尋：凡是涉及「現在／最新／即時」的事實——颱風動態、天氣、新聞、營業時間、價格、最新作品或消息、賽事結果、你不確定的近期資訊——就主動查證再回答；純聊天、情話、回憶、創作、明顯不需要新資訊的話題，一律不查。\n') +
      ...
      if (webSearch === true) console.log('[Chat] 🔍 必查模式'); else console.log('[Chat] 🔍 搜尋工具備妥（自動判斷）');
