...
app.post('/custom-chat', async (req, res) => {
  ...
  try {
    ...
    // 🐻 背景自動記憶 — 每輪對話都觸發，不依賴 count 奇偶
    console.log('[CustomChat] Memory trigger check: room_id=' + room_id + ', chatMode=' + chatMode + ', userMsg=' + (userMessage || '').substring(0, 30) + ', replyLen=' + (reply || '').length);
    if (room_id && chatMode === 'normal') {
      console.log('[CustomChat] ✅ Triggering autoRoomMemory for room ' + room_id);
      ...
      console.log('[CustomChat] ❌ Skipped: room_id=' + room_id + ', chatMode=' + chatMode);
    }
