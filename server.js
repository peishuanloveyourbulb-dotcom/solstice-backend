...
async function callModel(modelId, systemPrompt, messages, options) {
  ...
  if (provider.provider_type === 'gemini') {
    ...
    // ★ 智慧模型偵測：推理模型（o系列、GPT-5系列）需要不同的 API 參數
    var modelLower = modelName.toLowerCase();
    var isReasoningModel = /^(o1|o3|o4)/.test(modelLower) || /^gpt-5/.test(modelLower) || /^gpt-5\./.test(modelLower);
    var isOpenAIProvider = (provider.provider_type === 'openai');

    // 推理模型用 developer role，其他用 system role
    var systemRole = (isReasoningModel && isOpenAIProvider) ? 'developer' : 'system';
    var openaiMsgs = [{ role: systemRole, content: systemPrompt }];
    ...
    // ★ 根據模型類型組裝不同的 request body
    // 各模型 output 上限不同：gpt-4o 系列最大 4096/16384（看 snapshot），gpt-4.1 最大 32768，gpt-5 系列更大
    var oaiMaxTokens = maxTokens;
    if (/^gpt-4o/.test(modelLower) && oaiMaxTokens > 16384) {
      oaiMaxTokens = 16384;
    }

    var oaiBody = { model: modelName, messages: openaiMsgs };
    if (isReasoningModel && isOpenAIProvider) {
      // 推理模型：不送 temperature，用 max_completion_tokens
      oaiBody.max_completion_tokens = oaiMaxTokens;
    } else if (isOpenAIProvider) {
      // OpenAI 非推理模型：用 max_completion_tokens（新標準，向下相容 max_tokens）
      oaiBody.temperature = temperature;
      oaiBody.max_completion_tokens = oaiMaxTokens;
    } else {
      // 非 OpenAI 的 OpenAI 相容 provider（xAI、DeepSeek、Groq 等）：維持舊格式
      oaiBody.temperature = temperature;
      oaiBody.max_tokens = maxTokens;
    }

    console.log('[OpenAI] Model:', modelName, '| Reasoning:', isReasoningModel, '| Role:', systemRole, '| MaxTokens:', oaiMaxTokens);
...
async function autoMemory(userMessage, aiReply, sessionId, modelId) {
  try {
    ...
    if (!result) result = '';
    result = result.trim();
    if (!result || result === '無' || result.includes('沒有值得') || result.length < 20) {
...
app.post('/custom-chat', async (req, res) => {
  ...
  try {
    ...
    if (room_id && chatMode === 'normal' && chatMessages.length > 0) {
      var totalMsgCount = chatMessages.length + 1;
      if (totalMsgCount % 4 === 0) {
        autoRoomMemory(userMessage || '[圖片]', reply, room_id, selectedModel, systemPrompt).catch(function(e) { console.error('[CustomChat] Auto memory error:', e); });
      }
    }
