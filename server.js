...
// ==========================================
//  callModelWithFallback：Anthropic 內部 fallback
// ==========================================
async function callModelWithFallback(modelId, systemPrompt, messages, options) {
  ...
  // 第二層：Anthropic 內部穩定模型
  var stableList = ['claude-sonnet-4-6', 'claude-haiku-4-5-20251001'];
  var originalModelName = found ? found.modelName : '';

  for (var i = 0; i < stableList.length; i++) {
    if (stableList[i] === originalModelName) continue;
    try {
      console.log('[Fallback] 嘗試: ' + stableList[i]);
      var fbResult = await callModel(stableList[i], systemPrompt, messages, options);
      if (fbResult && fbResult.text && fbResult.text.trim()) {
        console.log('[Fallback] ✓ 成功: ' + stableList[i]);
        fbResult.actualModel = stableList[i];
        return fbResult;
      }
    } catch (fbErr) {
      console.log('[Fallback] ' + stableList[i] + ' 也失敗：' + fbErr.message);
    }
  }

  console.log('[Fallback] 所有嘗試都失敗了');
  return { text: null };
}
...
async function compressMemory(sessionId, settings, modelId) {
  try {
    ...
    return { summary: summary, compressed: toCompress.length };
  } catch (err) { 
...
app.post('/compress', async (req, res) => {
  try {
    ...
    res.json({ ok: true, summary: result.summary, compressed: result.compressed });
  } catch (e) { 
