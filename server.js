...
async function callModel(modelId, systemPrompt, messages, options) {
  ...
  if (provider.provider_type === 'openai') {
    ...
    // === Thinking 模式（兩層退路）===
    // 新模型(4.7+): adaptive + display:summarized（必須，否則思考內容為空）
    // 舊模型(4.6/4.5): enabled + budget_tokens
    ...
    if (useThinking) {
      if (hasQuirk(modelName, 'no_adaptive_thinking')) {
        ...
        claudeBody.thinking = { type: 'adaptive', budget_tokens: 10000, display: 'summarized' };
