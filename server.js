...
app.post('/capsules/generate', async (req, res) => {
  ...
  try {
    ...
    var loveMsg = '';
    try {
      loveMsg = await callModel(selectedModel, SOLSTICE_SOUL, [{ role: 'user', content: capsulePrompt }], { temperature: 1.2, maxTokens: 200 });
    } catch (genErr) {
    ...
    if (loveMsg.length > 200) loveMsg = loveMsg.substring(0, 200);
