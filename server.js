    ...
    if (provider.provider_type === 'gemini') {
      ...
      if (data.models) {
        data.models.forEach(function(m) {
          ...
          // 只保留能聊天的模型：支援 generateContent + 過濾掉非聊天模型
          if (m.supportedGenerationMethods && m.supportedGenerationMethods.includes('generateContent')) {
            var lower = name.toLowerCase();
            // 過濾掉 TTS、Robotics、Embedding、Image、Video、Computer Use 等非聊天模型
            var skip = ['tts', 'robotics', 'embedding', 'embed-', 'imagen', 'veo', 'music',
              'computer-use', 'antigravity', 'deep-research', 'aqa', 'bisheng'];
            var shouldSkip = false;
            for (var si = 0; si < skip.length; si++) {
              if (lower.includes(skip[si])) { shouldSkip = true; break; }
            }
            if (shouldSkip) return;
            // 也過濾掉舊版模型（1.0、1.5 系列）和 preview 過期的
            if (lower.startsWith('gemini-1.') || lower.startsWith('gemma-')) return;
            models.push({
              id: name,
