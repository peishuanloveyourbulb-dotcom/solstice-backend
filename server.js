...
app.post('/custom-chat', async (req, res) => {
  ...
  try {
    ...
    // 🐻 背景自動記憶（用當前 session_group 的實際訊息數）
    if (room_id && chatMode === 'normal') {
      try {
        var { count: sgMsgCount } = await supabase.from('custom_messages')
          .select('id', { count: 'exact', head: true })
          .eq('room_id', room_id)
          .eq('session_group', sg);
        // 每 2 筆 = 每輪對話（user+assistant 各一筆）觸發一次，與主框同步
        if (sgMsgCount && sgMsgCount % 2 === 0) {
        ...
        console.log('[CustomChat] Room', room_id, 'sg', sg, 'msgCount:', sgMsgCount, 'trigger:', !!(sgMsgCount && sgMsgCount % 2 === 0));
      } catch (countErr) { console.error('[CustomChat] Count error:', countErr); }
    }
