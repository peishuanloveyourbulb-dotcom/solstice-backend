...
async function callModel(modelId, systemPrompt, messages, options) {
  ...
  if (provider.provider_type === 'gemini') {
    ...
    if (enableThinking) {
      ...
      claudeBody.max_tokens = 16000; // 思考模式需要更大的 max_tokens
      // thinking 模式不支援 temperature
    } else {
    ...
    // ★ 解析回傳：分離 thinking 和 text blocks
    // Claude 4 模型的 thinking block 用 type='thinking', 內容在 .thinking 欄位
    // Claude 3.7 Sonnet 的 thinking block 內容在 .text 欄位（但 type 仍是 'thinking'）
    var claudeText = '';
    var claudeThinking = '';
    if (data3.content) {
