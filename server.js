...
app.post('/custom-rooms/:id/messages', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('custom_messages')
      .insert({ room_id: req.params.id, role: role, content: content }).select().single();
    if (error) throw error;
    res.json(data);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

app.delete('/custom-rooms/:id/messages', async (req, res) => {
  try {
    const { error } = await supabase.from('custom_messages').delete().eq('room_id', req.params.id);
    if (error) throw error;
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/custom-rooms/:id/memories', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('custom_memories')
      .insert({ room_id: req.params.id, summary: summary }).select().single();
    if (error) throw error;
    res.json(data);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/custom-chat', async (req, res) => {
  const { model, messages, mode, room_id } = req.body;
  ...
  try {
    ...
    console.log('[CustomChat] Model:', selectedModel, '| Mode:', chatMode, '| Room:', room_id || 'none',
      '| System:', systemPrompt.length, 'chars | Messages:', chatMessages.length);
    ...
    if (room_id) {
      try {
        await supabase.from('custom_messages').insert([
          { room_id: room_id, role: 'user', content: userMessage },
          { room_id: room_id, role: 'assistant', content: reply }
        ]);
      } catch (saveErr) { console.error('[CustomChat] Save messages error:', saveErr); }
    }

    res.json({ reply: reply, mode: chatMode });
