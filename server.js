...
app.patch('/sessions/:id', async (req, res) => {
  try {
    ...
    var updates = {};
    if (name !== undefined) { updates.Name = name; updates.updated_at = new Date().toISOString(); }
    if (pinned !== undefined) {
    ...
    // 直接用 https 打 Supabase REST API，繞過 JS client 的 .update() bug
    var body = JSON.stringify(updates);
    var parsed = new URL(SUPABASE_URL + '/rest/v1/sessions?id=eq.' + req.params.id);
    var result = await new Promise(function(resolve, reject) {
      var r = require('https').request({
        ...
        resp.on('end', function() {
          ...
          catch(e) { reject(new Error('Parse error: ' + text.substring(0, 200))); }
        });
      });
      r.on('error', reject);
      r.write(body);
