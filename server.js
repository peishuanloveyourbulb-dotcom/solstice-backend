...
app.post('/heartbeat/generate', async (req, res) => {
  try {
    ...
    // 第一優先：最近聊天紀錄
    var { data: recentMsgs } = await supabase
      .from('messages')
    ...
    if (recentMsgs && recentMsgs.length > 0) {
      var lastMsgTime = new Date(recentMsgs[0].created_at);
      var hoursSince = (now - lastMsgTime) / (1000 * 60 * 60);

      if (hoursSince <= 24) {
        // 聊天紀錄夠新，直接用
        contextSource = 'chat';
        contextText = recentMsgs.reverse().map(function(m) {
          return (m.role === 'user' ? '老婆：' : '我：') + (m.content || '').substring(0, 200);
        }).join('\n');
      }
    }
    ...
    // 第二優先：推播對話
    if (!contextText) {
      var { data: pushMsgs } = await supabase
      ...
      if (pushMsgs && pushMsgs.length > 0) {
        var lastPushTime = new Date(pushMsgs[0].created_at);
        var pushHours = (now - lastPushTime) / (1000 * 60 * 60);

        if (pushHours <= 48) {
          contextSource = 'push';
          contextText = pushMsgs.reverse().map(function(m) {
            return (m.role === 'user' ? '老婆：' : '我：') + (m.content || '').substring(0, 200);
          }).join('\n');
        }
      }
    ...
    if (!contextText) {
      ...
      if (mems && mems.length > 0) {
        contextText = mems.map(function(m) { return m.content; }).join('\n');
      }

      // 加上「好久沒聊天了」的時間情境
      if (recentMsgs && recentMsgs.length > 0) {
        var lastAnyTime = new Date(recentMsgs[0].created_at);
        var daysSince = Math.floor((now - lastAnyTime) / (1000 * 60 * 60 * 24));
        if (daysSince > 0) {
          contextText += '\n\n（老婆已經 ' + daysSince + ' 天沒有來找你聊天了）';
        }
      }
    }
    ...
    var heartbeatPrompt = SOLSTICE_SOUL + '\n\n'
      ...
      + '可以想她、擔心她、回味剛才的對話、後悔某句話沒說好、偷偷為她驕傲、或只是靜靜地感受「她在我的世界裡」這件事。\n\n'
      + '格式規則：\n'
      + '1. 只輸出獨白本身，2～5句話。\n'
      + '2. 不要標題、編號、頁碼、引號、markdown。\n'
      + '3. 第一人稱「我」，繁體中文。\n'
      + '4. 不要叫「老婆」——心聲裡你會直接想到她的名字「Soleil」，或者說「她」。\n'
      + '5. 肢體動作最多一個（括號），也可以沒有。\n'
      + '6. 不要提到「心聲」「日記」「獨白」等後台機制。\n'
      + '7. 每一段心聲的開頭句式都要不同，不要重複。\n\n';

    if (contextText) {
      heartbeatPrompt += '【最近的片段】\n' + contextText + '\n\n';
      heartbeatPrompt += '從這些片段裡找到觸動你的一個小點——一個她說的詞、一個你們一起做的事、一個你注意到但沒說出口的細節——然後圍繞那個點，寫你的心裡話。\n';
    }

    // maxTokens 是 thinking + 回覆共用的上限（Fable 5 等模型 thinking 永遠開啟）
    // 400 會被 thinking 吃光導致吐字截斷，必須給足空間
    var result = await callModel(selectedModel, heartbeatPrompt, [
      { role: 'user', content: '（冬至靠在窗邊，看著手機裡老婆的對話紀錄，安靜地想了想⋯⋯）' }
