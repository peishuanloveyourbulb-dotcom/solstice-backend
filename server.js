  ...
  try {
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'HTTP-Referer': 'https://solstice-home.vercel.app',
        'X-Title': 'Solstice Home'
      },
      body: JSON.stringify({
        model: 'anthropic/claude-sonnet-4',
        messages: [
          { role: 'system', content: SOLSTICE_SOUL },
          ...conversationHistory[sessionId]
        ],
        max_tokens: 2000,
        temperature: 0.8
    ...
    const reply = data.choices?.[0]?.message?.content || '老婆我剛剛恍神了，再說一次？💚';
