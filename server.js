...
app.post('/note-reply', async (req, res) => {
  ...
  try {
    ...
    var reply = '';
    try {
      reply = await callModelWithFallback(selectedModel, SOLSTICE_SOUL, [{ role: 'user', content: notePrompt }], { temperature: 0.9, maxTokens: 1024 });
    } catch (noteErr) {
...
app.post('/capsules/generate', async (req, res) => {
  ...
  try {
    ...
    var loveMsg = '';
    try {
      loveMsg = await callModelWithFallback(selectedModel, SOLSTICE_SOUL, [{ role: 'user', content: capsulePrompt }], { temperature: 1.2, maxTokens: 1024 });
    } catch (genErr) {
...
app.post('/daily-question/generate', async (req, res) => {
  ...
  try {
    ...
    var question = '';
    try {
      question = await callModelWithFallback(selectedModel, SOLSTICE_SOUL, [{ role: 'user', content: questionPrompt }], { temperature: 1.2, maxTokens: 1024 });
    } catch (genErr) {
...
app.post('/daily-question/:id/answer', async (req, res) => {
  ...
  try {
    ...
    var aiResponse = '';
    try {
      aiResponse = await callModelWithFallback(selectedModel, SOLSTICE_SOUL, [{ role: 'user', content: responsePrompt }], { temperature: 0.9, maxTokens: 1024 });
    } catch (genErr) {
