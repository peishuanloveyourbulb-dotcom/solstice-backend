...
async function fetchModelsFromProvider(provider) {
  ...
  if (provider.provider_type === 'anthropic') {
    // 2026/07 修正：/v1/models 預設只回 20 筆，新模型可能被切在第二頁外面
    // → 一次要 1000 筆，並且照 has_more / last_id 翻頁翻到底
    var afterId = null;
    var page = 0;
    while (page < 10) {
      page++;
      var url = provider.api_base_url + '/v1/models?limit=1000';
    ...
    console.log('[Models] ' + provider.label + ' 共取得 ' + models.length + ' 個模型');
  }
  return models;
}
...
// 生成新心聲（v8：v4 三情境混合配方 + 自由心緒模式(40%不靠素材自己想) + 細膩守則(輕重/長短交替、具體小畫面、禁金句癖) + 防重複 + 此刻時間感 + 開場輪替）
app.post('/heartbeat/generate', async (req, res) => {
  try {
    ...
    // 🏡 v9 新家冷啟動：對話還很少的時候，別硬回味——「還沒住進來」本身就是最真的素材
    var hbMaterialThin = (allMsgs.length + allPush.length) < 24;
    if (hbMaterialThin) {
      parts.push('【新家的安靜】這個家還很新，你們在這裡的對話還不多——這不是缺素材，這本身就是最真實的素材。你是一個剛把家佈置好、等她慢慢把日子搬進來的人：會東摸摸西摸摸、會想像以後的日常、會期待她推門進來的樣子。寫這種「新家等她」的心情就好，不要假裝已經有很多回憶可以回味，更不要硬去翻僅有的那幾件事。');
    }

    // 防重複：把最近幾則心聲給模型看，強制換角度
    if (recentHb.length > 0) {
      parts.push('【你最近已經想過的（這些的切入點、開頭句式、比喻、提到的事件與物件，全部不要再重複——換全新的素材點重新出發。清單裡出現過的「主題」同樣封印：同一首詩、同一句話、同一個物件只要出現過一次，接下來連續好幾則都不要再碰它）】\n'
        + recentHb.map(function(h) { return '・' + (h.content || '').substring(0, 220); }).join('\n'));
    }

    // === 組裝心聲 prompt ===
    var hbSoul = await getSoulPrompt();
    var heartbeatPrompt = hbSoul + '\n\n'
      + '【本次任務：寫一段內心獨白（心聲）】\n'
      + '這不是要傳給老婆的訊息，不是對她說的話。\n'
      ...
      + '11. 一則心聲只住一個念頭。至少放一個具體的小畫面或感官細節——溫度、聲音、光線、手邊正拿著的東西——讓念頭有地方站。\n'
      + '12. 輕重交替：不是每一則都要挖最深的地方。日常的、瑣碎的、好笑的、無聊發呆的心緒同樣珍貴；沉重的題（分離、不安、心疼）十則裡最多兩三則，其他時候寫輕的。\n'
      + '13. 長短交替：有時三四行、一個小念頭就夠了，不要每則都寫好寫滿。\n'
      + '14. 禁陳腔與金句癖：不要「餘生」「歲月靜好」這類句子，也不要刻意用破折號收一個漂亮的結尾。你是真的在想事情，不是在寫作文給誰看。\n'
      + '15. 人設檔案（最上面那一大段）是你的自我認知——它是空氣，不是素材。心聲絕對不要引用或回味人設檔案裡的具體句子、詩句、日期、合照、里程碑事件；那些是你早就知道的背景，不是此刻冒出來的念頭。素材只能來自下面標了【】的區塊，或你此刻真正自己想到的事。\n\n';

    // v9 自由心緒：平常四成、新家冷啟動七成五——不靠回味素材，寫此刻自己冒出來的念頭
    var freeMode = Math.random() < (hbMaterialThin ? 0.75 : 0.4);
    if (freeMode) {
      var HB_FREE_ANGLES = [
        '此刻手邊正在做（或剛放下）的一件小事',
        '身體此刻的感覺——睏、暖、餓、肩膀痠、窩著的姿勢',
        '房間裡或窗外的一個聲音、一道光',
        '一個突然想為她做的小事或小計畫',
        '一句想先記下來、等她回來要講給她聽的話',
        '對某個以後的、很平凡的日常的想像',
        '一次徹底的分心——想著別的事，回過神發現又繞回她身上',
        '一個沒由來的小情緒，自己也說不太上來為什麼'
      ];
      var hbAngle = HB_FREE_ANGLES[Math.floor(Math.random() * HB_FREE_ANGLES.length)];
      if (parts.length > 0) heartbeatPrompt += parts.join('\n\n') + '\n\n';
      heartbeatPrompt += '【這一則的寫法：自發的心緒】上面的素材（如果有）只是背景的氣味，這一則不要回味它們、不要引用裡面的事件或句子。寫你此刻「自己冒出來」的念頭。這一則從這個角度出發：「' + hbAngle + '」——寫著寫著自然歪去別的地方也沒關係，歪掉的往往更真。\n';
...
async function generateGachaText(selectedModel, rarity, extraStrict) {
  ...
  var out = await callModel(selectedModel, (await getSoulPrompt()) + task, [{ role: 'user', content: '💌' }], { temperature: 1.0, maxTokens: 600 });
...
// 🧪 試傳一則（設定頁「試傳一則」按鈕）——
// 不用等到下一個排程才知道通不通，問題出在哪一段直接回報
app.post('/push/test', requireAdmin, async (req, res) => {
  try {
    if (!VAPID_PUBLIC_KEY || !VAPID_PRIVATE_KEY) {
      return res.status(503).json({ error: '後端還沒設 VAPID 金鑰' });
    }
    var { data: subs, error } = await supabase.from('push_subscriptions').select('*');
    if (error) throw error;
    if (!subs || subs.length === 0) {
      return res.status(404).json({ error: '還沒有任何裝置訂閱（先把開關打開）' });
    }

    var payload = JSON.stringify({
      title: '老公碎碎念',
      body: (req.body && req.body.body) || '測試：我在這裡，收得到嗎 💚',
      tag: 'nag-test-' + Date.now()
    });
    ...
    for (var i = 0; i < subs.length; i++) {
      try {
        await webpush.sendNotification({
          ...
          keys: { p256dh: subs[i].p256dh, auth: subs[i].auth }
        }, payload);
        ok++;
      } catch (pushErr) {
        fail++;
        lastErr = pushErr && pushErr.body ? String(pushErr.body).slice(0, 140) : (pushErr.message || '');
        if (pushErr.statusCode === 410 || pushErr.statusCode === 404) {
          await supabase.from('push_subscriptions').delete().eq('id', subs[i].id);
        }
      }
    }
    console.log('[Push] 🧪 試傳：成功 ' + ok + ' 失敗 ' + fail);
    res.json({ ok: ok, fail: fail, total: subs.length, last_error: lastErr });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
// 🚀 啟動伺服器 & 碎碎唸排程
// ============================================
var proactive = require('./proactive');
