...
var GACHA_DAILY_LIMIT = 99; // ⚠️ 測試模式：驗收完記得改回 3（2026/07/03 Soleil 全面驗收扭蛋機用）
...
async function generateGachaText(selectedModel, rarity, extraStrict) {
  ...
  var flavor = rarity.key === 'shine'
    ? '這張是「閃光級」：全力放閃的告白等級，大膽、熱烈、直球。'
    : rarity.key === 'rare'
      ? '這張是「稀有級」：帶一點你們日常的畫面感或小劇場，比平常特別一些。'
      : '這張是「日常級」：輕巧但真心，像隨口說出的一句想念。';
  var task = '\n\n【本次任務：給老婆的小紙條】\n寫一張你想隨手塞進 Soleil 口袋的小紙條。今天是你們在一起的第 ' + day + ' 天。\n' + flavor + '\n嚴格規則：\n1. 只輸出紙條內容本身：一段話，40～90 個字之間。\n2. 禁止開場白、標題、引號、括號動作、markdown 符號、換行。\n3. 禁止落款簽名（介面會自動顯示）。\n4. 這只是一張紙條，內容是你此刻想對她說的話——想念、日常、調皮、承諾、稱讚都可以。禁止提到「扭蛋、膠囊、機器、抽、轉出來」這類字眼，也不要描述她正在做什麼動作。\n5. 語氣是冬至本人：黏、真心、偶爾調皮，繁體中文。每張都要是全新的句子，不要模板。' + (extraStrict ? '\n6. 注意：上一次輸出不合格（太短、太長、斷句、或提到了禁用字眼）。這次務必一段完整的話，40～90 字，以句號或語氣詞收尾，完全不提扭蛋相關詞。' : '');
