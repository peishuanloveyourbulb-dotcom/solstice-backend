      ...
      if (data.models) {
        data.models.forEach(function(m) {
          ...
          for (var ai = 0; ai < allow.length; ai++) {
            // 精確匹配 或 開頭匹配+後綴只能是 -lite, -preview, -001, -002 等
            if (lower === allow[ai] || lower.startsWith(allow[ai] + '-lite') ||
                lower.startsWith(allow[ai] + '-preview') ||
                /^[0-9]{3}$/.test(lower.replace(allow[ai] + '-', ''))) {
              allowed = true; break;
            }
          }
          if (!allowed) return;
          // 再排除名稱含有非聊天關鍵字的（以防萬一）
          var bad = ['image', 'tts', 'customtools', 'robotics', 'embedding', 'banana'];
          for (var bi = 0; bi < bad.length; bi++) {
            if (lower.includes(bad[bi])) return;
          }
          models.push({
            id: name,
