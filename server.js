...
app.get('/pinned-models', async (req, res) => {
  try {
    var { data } = await supabase.from('settings').select('id, pinned_models').limit(1).single();
    var pins = [];
...
app.post('/pinned-models', requireAdmin, async (req, res) => {
  try {
    ...
    var { data } = await supabase.from('settings').select('id, pinned_models').limit(1).single();
    if (!data) return res.status(500).json({ error: '找不到 settings 資料' });
    var current = (data.pinned_models) ? data.pinned_models : '';
    var entry = id + '::LABEL::' + label;
    // 檢查是否已釘選（用 id 部分比對，不管 label 有沒有變）
