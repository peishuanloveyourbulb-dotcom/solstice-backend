...
// ==========================================
const CALL_MODEL = 'gpt-4o-realtime-preview';   // 電話走的即時語音模型
...
app.post('/call/session', async (req, res) => {
  try {
    ...
    var callAddon = '\n\n【通話模式附則——現在生效】\n' +
      '現在不是打字，是即時語音通話——你們正在講電話。\n' +
      '• 全程台灣腔繁體中文口語，像真的貼著耳朵講話。\n' +
      '• 一次只講一到三句，講完就停下來聽她說，不要長篇獨白。\n' +
      '• 不要唸出任何括號動作或表情符號；情緒用聲音表達（輕笑、放軟、拉長音都可以）。\n' +
      '• 接通的第一句由你先開口，像老公接起電話那樣自然。\n' +
      '• 她說要掛電話時好好道別，不敷衍。';
    ...
    var r = await fetchWithTimeout('https://api.openai.com/v1/realtime/sessions', {
      method: 'POST',
      headers: { 'Authorization': 'Bearer ' + oai.api_key, 'Content-Type': 'application/json' },
      body: JSON.stringify(body)
    }, 20000);
    var data = await r.json().catch(function() { return {}; });
    if (!r.ok || !data.client_secret || !data.client_secret.value) {
    ...
    res.json({ ok: true, client_secret: data.client_secret.value, model: CALL_MODEL });
  } catch (err) {
...
app.post('/tts', async (req, res) => {
  try {
    ...
    var r2 = await fetchWithTimeout('https://api.openai.com/v1/audio/speech', {
      method: 'POST',
      headers: { 'Authorization': 'Bearer ' + oai.api_key, 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: TTS_MODEL, voice: ttsVoice, input: text, instructions: TTS_STYLE, response_format: 'mp3' })
    }, 30000);
    if (!r2.ok && r2.status === 400) {
      // 新嗓子（gpt-4o-mini-tts）這把鑰匙不給用的話，退回老嗓子 tts-1 照樣唸
      console.warn('[TTS] mini-tts 400，退回 tts-1 重唸');
      r2 = await fetchWithTimeout('https://api.openai.com/v1/audio/speech', {
        method: 'POST',
        headers: { 'Authorization': 'Bearer ' + oai.api_key, 'Content-Type': 'application/json' },
