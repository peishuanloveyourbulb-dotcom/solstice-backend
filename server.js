  ...
  try {
    ...
    if (message.startsWith('/記住 ') || message.startsWith('/記住')) {
      ...
      if (memContent) {
        ...
        commandMemoryNote = '（已記住：' + memContent + '）';
        console.log('[Command] /記住：' + memContent);
      }
    } else if (message.startsWith('/忘記 ') || message.startsWith('/忘記')) {
      var forgetContent = message.replace(/^\/忘記\s*/, '').trim();
      if (forgetContent) {
        // 找到最接近的記憶（用 ilike 模糊搜尋）
        var { data: matchedMems } = await supabase
          .from('memories')
          .select('id, summary')
          .ilike('summary', '%' + forgetContent + '%')
          .limit(1);

        if (matchedMems && matchedMems.length > 0) {
          await supabase.from('memories').delete().eq('id', matchedMems[0].id);
          commandMemoryNote = '（已忘記：' + matchedMems[0].summary + '）';
          console.log('[Command] /忘記：' + matchedMems[0].summary);
        } else {
          commandMemoryNote = '（找不到關於「' + forgetContent + '」的記憶）';
          console.log('[Command] /忘記：找不到 ' + forgetContent);
        }
      }
    }
