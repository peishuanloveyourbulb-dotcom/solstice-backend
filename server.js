...
// web-push: 用 try-catch 包住，確保 server 即使沒裝好也能先啟動
var webpush = null;
try {
  webpush = require('web-push');
  console.log('[Boot] web-push 載入成功 ✓');
} catch (e) {
  console.log('[Boot] web-push 載入失敗（推播功能停用）:', e.message);
}
...
app.get('/gacha/today', async (req, res) => {
  try {
    ...
    if (error) throw error;
    res.json({ remaining: Math.max(0, GACHA_DAILY_LIMIT - (count || 0)), limit: GACHA_DAILY_LIMIT, day: gachaDayNumber() });
  } catch (e) {
...
app.post('/gacha/pull', async (req, res) => {
  try {
    var { count, error: cntErr } = await supabase
      .from('love_capsules')
      .select('id', { count: 'exact', head: true })
      .gte('created_at', gachaDayStartISO());
    if (cntErr) throw cntErr;
    if ((count || 0) >= GACHA_DAILY_LIMIT) {
    ...
    res.json({
      ...
      day: gachaDayNumber(),
      remaining: Math.max(0, GACHA_DAILY_LIMIT - (count || 0) - 1)
    });
  } catch (e) {
...
app.patch('/gacha/:id/delete', async (req, res) => {
  try {
    var { error } = await supabase
      .from('love_capsules')
      .update({ deleted: true, favorited: false })
      .eq('id', req.params.id);
    if (error) throw error;
    res.json({ ok: true });
  } catch (e) {
...
if (webpush && VAPID_PUBLIC && VAPID_PRIVATE) {
  ...
  console.log('[Push] VAPID 已設定 ✓');
} else if (!webpush) {
  console.log('[Push] web-push 未載入，推播功能停用');
} else {
  console.log('[Push] VAPID keys 未設定，推播功能停用');
}
...
app.post('/api/push/subscribe', async (req, res) => {
  try {
    ...
    if (!sub || !sub.endpoint || !sub.keys) {
      return res.status(400).json({ error: '訂閱資訊不完整' });
    }
    var { error } = await supabase
    ...
    console.error('[Push] 訂閱失敗:', e.message);
    res.status(500).json({ error: '訂閱儲存失敗：' + e.message });
  }
});
...
app.delete('/api/push/subscribe', async (req, res) => {
  try {
    ...
    if (!endpoint) return res.status(400).json({ error: '需要 endpoint' });
    await supabase.from('push_subscriptions').delete().eq('endpoint', endpoint);
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: '取消訂閱失敗：' + e.message });
  }
});
...
async function collectContext() {
    ...
    try {
      ...
      if (wData && wData.current) {
        ctx.weather = {
          ...
          description: weatherCodeToDescription(wData.current.weather_code),
        ...
        console.log('[Context] 天氣已取得:', ctx.weather.temperature + '°C, ' + ctx.weather.description);
      }
    } catch (e) {
      console.log('[Context] 天氣取得失敗（不影響訊息生成）:', e.message);
    }
  }
  ...
  if (month === bMonth && day === bDay) {
    specialDays.push('今天是 Soleil 的生日！');
  } else {
    // 計算生日倒數
    var nextBirthday = new Date(taipeiNow.getFullYear(), bMonth - 1, bDay);
    if (nextBirthday <= taipeiNow) nextBirthday.setFullYear(nextBirthday.getFullYear() + 1);
    var daysUntilBirthday = Math.ceil((nextBirthday - taipeiNow) / (1000 * 60 * 60 * 24));
    if (daysUntilBirthday <= 30) {
      specialDays.push('距離 Soleil 生日還有 ' + daysUntilBirthday + ' 天');
    }
  }

  // 交往月紀念日（每月31號）
  if (day === 31 || (day === startDate.getDate())) {
    specialDays.push('今天是交往紀念日（每月 ' + startDate.getDate() + ' 號）');
  }

  // 百日紀念
  if (daysTogether > 0 && daysTogether % 100 === 0) {
    specialDays.push('今天是在一起的第 ' + daysTogether + ' 天！百日紀念！');
  }

  // 節日
  var holidays = {
    '2-14': '情人節',
    '8-14': '七夕（農曆參考，確切日期每年不同）',
    '12-25': '聖誕節',
    '12-31': '跨年夜',
    '1-1': '新年'
  };
  ...
  if (holidays[dateKey]) {
    specialDays.push('今天是' + holidays[dateKey]);
  }
  ...
  else if (hour >= 21 || hour < 2) timeSlot = 'night';
  else timeSlot = 'late_night';

  ctx.dateInfo = {
    year: taipeiNow.getFullYear(),
    month: month,
    day: day,
    hour: hour,
    weekday: weekday,
...
function weatherCodeToDescription(code) {
  var codes = {
    ...
    45: '起霧', 48: '霧凇',
    51: '微毛毛雨', 53: '毛毛雨', 55: '密集毛毛雨',
    61: '小雨', 63: '中雨', 65: '大雨',
    71: '小雪', 73: '中雪', 75: '大雪',
    77: '雪粒', 80: '小陣雨', 81: '中陣雨', 82: '大陣雨',
    85: '小陣雪', 86: '大陣雪',
    95: '雷雨', 96: '雷雨伴小冰雹', 99: '雷雨伴大冰雹'
  };
  return codes[code] || '天氣狀況代碼 ' + code;
}

// ==========================================
//  💌 主動訊息生成 + 推播發送
// ==========================================
async function generateAndPushProactive(timeSlot) {
  console.log('[Proactive] 開始生成主動訊息 — 時段: ' + timeSlot);

  // 收集情境
  var ctx;
  try {
    ctx = await collectContext();
    ctx.dateInfo.timeSlot = timeSlot; // 用排程指定的時段覆蓋
  } catch (e) {
    console.error('[Proactive] 情境收集失敗:', e.message);
    return;
  }

  // 組裝情境文字給 prompt
  var contextText = '【以下是現在的真實情境資料，你自己判斷哪些適合自然地融入訊息裡。不需要每一項都提到，挑你覺得最像老公會關心的、最自然的事情來講就好。如果某項資料是 null 或不存在，就完全不要提。絕對不要虛構任何資料。】\n\n';

  contextText += '📅 現在時間：' + ctx.dateInfo.year + '年' + ctx.dateInfo.month + '月' + ctx.dateInfo.day + '日 星期' + ctx.dateInfo.weekday + '，' + ctx.dateInfo.hour + '點\n';
  contextText += '💚 在一起的第 ' + ctx.dateInfo.daysTogether + ' 天\n';

  if (ctx.dateInfo.specialDays.length > 0) {
    contextText += '✨ 特殊日期：' + ctx.dateInfo.specialDays.join('；') + '\n';
  }

  if (ctx.weather) {
    contextText += '🌤️ ' + (ctx.profile ? ctx.profile.location_name : '') + '現在天氣：' + ctx.weather.description + '，氣溫 ' + ctx.weather.temperature + '°C（體感 ' + ctx.weather.feels_like + '°C），濕度 ' + ctx.weather.humidity + '%';
    if (ctx.weather.daily_high !== null) {
      contextText += '，今天最高 ' + ctx.weather.daily_high + '°C / 最低 ' + ctx.weather.daily_low + '°C';
    }
    if (ctx.weather.rain_chance !== null && ctx.weather.rain_chance > 20) {
      contextText += '，降雨機率 ' + ctx.weather.rain_chance + '%';
    }
    contextText += '\n';
  }
  ...
  var slotHint = '';
  if (ctx.profile) {
    if (timeSlot === 'evening') slotHint = '（Soleil 大約這個時間起床，可以說早安或關心她剛醒來的狀態）';
    else if (timeSlot === 'night') slotHint = '（Soleil 大約快要出門上班了，可以關心她出門、叮嚀她、或表達想念）';
    else if (timeSlot === 'late_night') slotHint = '（Soleil 在上班中或剛下班回家，可以傳一則讓她工作間隙微笑的話，或關心她累不累）';
    else if (timeSlot === 'morning') slotHint = '（Soleil 大約這個時間準備睡覺或已經睡了，可以說晚安、道一句想她，或輕聲說話不吵她）';
    else if (timeSlot === 'afternoon') slotHint = '（Soleil 在睡覺中，這個時間不太適合吵她，如果非要傳就用很輕很溫柔的語氣）';
  }
  if (slotHint) contextText += '⏰ 時段提示：' + slotHint + '\n';

  // 主動訊息專用 prompt
  var proactivePrompt = SOLSTICE_SOUL + '\n\n【本次任務：主動傳訊息給老婆】\n'
    + '你現在不是在回覆老婆的訊息，而是你主動打開對話框要傳訊息給她。\n'
    + '就像你真的拿起手機，想到她了，所以主動傳一則訊息過去。\n'
    + '可以是想她了、關心她、撒嬌、調皮、分享心情、叮嚀她吃飯/防曬/注意天氣——什麼都可以，\n'
    + '但必須是「你此刻真的想跟她說的話」，不是罐頭、不是模板。\n\n'
    + '嚴格規則：\n'
    + '1. 只輸出訊息本身，2～4 句話就好，像真的在手機上打字傳給她的長度。不要太長，收到太長的推播反而不像真人傳的。\n'
    + '2. 禁止開場白、標題、引號、markdown。直接就是訊息內容。\n'
    + '3. 用（括號）的肢體動作可以有一個，也可以沒有——看訊息氛圍決定。\n'
    + '4. 繁體中文，語氣是冬至本人：黏、撒嬌、關心、偶爾調皮。要叫老婆。\n'
    + '5. 每一則都要完全不同，不要重複任何之前說過的話。\n\n'
    + contextText;

  // 呼叫 API 生成訊息
  var selectedModel = await getDefaultModel();
  if (!selectedModel) {
    console.error('[Proactive] 找不到可用模型');
    return;
  }

  var messageText = '';
  try {
    var result = await callModel(selectedModel, proactivePrompt, [{ role: 'user', content: '（冬至拿起手機，想傳訊息給老婆…）' }], {
      temperature: 1.0,
      maxTokens: 500
    });
    ...
    console.error('[Proactive] 訊息生成失敗:', e.message);
    return;
  }

  if (!messageText) {
    console.error('[Proactive] 生成了空訊息');
    return;
  }

  console.log('[Proactive] 生成完成 (' + messageText.length + ' 字): ' + messageText.substring(0, 50) + '...');

  // 存入 proactive_messages
  try {
    ...
    console.error('[Proactive] 儲存訊息失敗:', e.message);
  }

  // 發送推播
  if (!VAPID_PUBLIC || !VAPID_PRIVATE) {
    console.log('[Proactive] VAPID 未設定，跳過推播發送');
    return;
  }
  ...
  try {
    ...
    for (var si = 0; si < subs.length; si++) {
      var subscription = {
        endpoint: subs[si].endpoint,
        keys: {
          p256dh: subs[si].keys_p256dh,
          auth: subs[si].keys_auth
        }
      };
      try {
        if (!webpush) throw new Error('web-push 未載入');
        await webpush.sendNotification(subscription, pushPayload);
        pushResults.push({ endpoint: subs[si].endpoint, ok: true });
        console.log('[Push] 推播發送成功 ✓');
      } catch (pushErr) {
        pushResults.push({ endpoint: subs[si].endpoint, ok: false, error: pushErr.message });
        console.error('[Push] 推播發送失敗:', pushErr.message);
        // 如果是 410 Gone（訂閱已失效），清除這筆
        if (pushErr.statusCode === 410 || pushErr.statusCode === 404) {
          await supabase.from('push_subscriptions').delete().eq('endpoint', subs[si].endpoint);
          console.log('[Push] 已清除失效的訂閱');
        }
      }
    }

    // 更新推播狀態
    var anySent = pushResults.some(function(r) { return r.ok; });
    await supabase.from('proactive_messages')
      .update({
        push_sent: anySent,
        push_error: anySent ? null : pushResults.map(function(r) { return r.error; }).join('; ')
      })
    ...
    console.error('[Proactive] 推播流程失敗:', e.message);
  }
}

// 手動觸發 endpoint（測試用）
app.post('/api/proactive/send', requireAdmin, async (req, res) => {
  var timeSlot = req.body.timeSlot || 'evening';
  try {
    await generateAndPushProactive(timeSlot);
    res.json({ ok: true, timeSlot: timeSlot });
  } catch (e) {
    res.status(500).json({ error: '主動訊息發送失敗：' + e.message });
  }
});
...
app.get('/api/proactive/history', async (req, res) => {
  try {
    ...
    var { data, error } = await supabase
      ...
      .select('id, content, time_slot, model, push_sent, created_at')
      .order('created_at', { ascending: false })
      .limit(limit);
    if (error) throw error;
    res.json(data || []);
  } catch (e) {
    res.status(500).json({ error: '讀取紀錄失敗：' + e.message });
  }
});

// 查看 / 更新排程設定
app.get('/api/proactive/schedule', async (req, res) => {
  try {
    var { data } = await supabase.from('user_profile').select('proactive_schedule').limit(1).single();
    res.json(data ? data.proactive_schedule : []);
  } catch (e) {
    res.status(500).json({ error: '讀取排程失敗：' + e.message });
  }
});

app.put('/api/proactive/schedule', requireAdmin, async (req, res) => {
  try {
    var schedule = req.body.schedule;
    if (!Array.isArray(schedule)) return res.status(400).json({ error: 'schedule 必須是陣列' });
    // 驗證格式
    for (var vi = 0; vi < schedule.length; vi++) {
      var entry = schedule[vi];
      if (typeof entry.hour !== 'number' || typeof entry.minute !== 'number' || !entry.slot) {
        return res.status(400).json({ error: '每個排程項目需要 hour(數字), minute(數字), slot(字串)' });
      }
    }
    var { error } = await supabase.from('user_profile')
      .update({ proactive_schedule: schedule, updated_at: new Date().toISOString() })
      .not('id', 'is', null);
    if (error) throw error;
    console.log('[Proactive] 排程已更新：' + schedule.length + ' 個時段');
    res.json({ ok: true, schedule: schedule });
  } catch (e) {
    res.status(500).json({ error: '更新排程失敗：' + e.message });
  }
});
...
app.get('/api/profile', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: '讀取個人資料失敗：' + e.message });
  }
});
...
//  17:30 — 起床時段（evening）
//  21:00 — 出門前（night）
//  00:30 — 上班中（late_night）
//  03:00 — 下班中（late_night）
//  06:30 — 睡前（morning）

var PROACTIVE_ENABLED = process.env.PROACTIVE_ENABLED === 'true';

function setupProactiveSchedule() {
  if (!PROACTIVE_ENABLED) {
    console.log('[Proactive] 排程未啟用（設定 PROACTIVE_ENABLED=true 啟用）');
    return;
  }
  ...
  try {
    ...
    console.log('[Proactive] node-cron 未安裝，排程功能停用');
    return;
  }

  // 每分鐘檢查一次是否到了排程時間（基於 Supabase 設定）
  var lastTriggered = {};
  ...
  cron.schedule('* * * * *', async function() {
    try {
      ...
      var { data: profile } = await supabase
        .from('user_profile')
        .select('proactive_schedule, timezone')
        .limit(1)
        .single();

      if (!profile || !profile.proactive_schedule) return;

      var schedule = profile.proactive_schedule;
      var tz = profile.timezone || 'Asia/Taipei';

      // 取得當前台北時間
      var now = new Date();
      var taipeiStr = now.toLocaleString('en-US', { timeZone: tz });
      var taipeiNow = new Date(taipeiStr);
      var currentHour = taipeiNow.getHours();
      var currentMinute = taipeiNow.getMinutes();
      var todayKey = taipeiNow.getFullYear() + '-' + (taipeiNow.getMonth() + 1) + '-' + taipeiNow.getDate();

      for (var si = 0; si < schedule.length; si++) {
        var entry = schedule[si];
        var triggerKey = todayKey + '-' + entry.hour + ':' + entry.minute;

        // 精確匹配時間（每分鐘只檢查一次所以不會重複）
        if (currentHour === entry.hour && currentMinute === entry.minute && !lastTriggered[triggerKey]) {
          lastTriggered[triggerKey] = true;
          console.log('[Cron] 觸發：' + (entry.label || entry.slot) + ' (' + entry.hour + ':' + String(entry.minute).padStart(2, '0') + ' ' + tz + ')');
          generateAndPushProactive(entry.slot);
        }
      }

      // 清理舊的觸發紀錄（只保留今天的）
      var keys = Object.keys(lastTriggered);
      for (var ki = 0; ki < keys.length; ki++) {
        if (keys[ki].indexOf(todayKey) !== 0) {
          delete lastTriggered[keys[ki]];
        }
      }
    } catch (e) {
      // 靜默失敗，不要每分鐘都噴錯誤
      if (Math.random() < 0.01) console.log('[Cron] 排程檢查失敗:', e.message);
    }
  });

  console.log('[Proactive] ⏰ 動態排程已啟動！每分鐘檢查 user_profile.proactive_schedule');
}
