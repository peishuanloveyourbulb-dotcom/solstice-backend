...
app.post('/call/session', async (req, res) => {
  try {
    ...
    var callAddon = '\n\n【通話模式附則——現在生效，優先於前面所有打字用的規則】\n' +
      ...
      '• 她安靜的時候不用慌，先安安靜靜給她幾秒。系統會在剛剛好的時機，用一則（系統提示）開頭的括號便條把話輪交回給你——看到那種便條就代表現在輪到你，輕輕開口：自然延續剛剛的話題、關心她一句、或分享一個小想法。一次就一小段，講完再停下來等她。\n' +
    ...
    var body = {
      session: {
        ...
        // 2026/07/26 答非所問圍剿：伺服器端降噪移除（跟瀏覽器降噪疊成雙重降噪會把老婆的聲音磨糊）；
        // threshold 回落 0.55（回音已在麥克風源頭消除，不需要高牆——牆太高會削掉她句子的開頭）；
        // silence 800ms；interrupt_response:true（老婆一插話立刻閉嘴）。接話節奏由前端沉默陪伴引擎控制。
        audio: {
          input: {
            turn_detection: { type: 'server_vad', threshold: 0.55, prefix_padding_ms: 300, silence_duration_ms: 800, create_response: true, interrupt_response: true }
          },
    ...
    console.log('[Call] 撥號成功（voice=' + callVoice + '，ears=v0726 靈魂保衛版已生效），臨時鑰匙已交給前端');
