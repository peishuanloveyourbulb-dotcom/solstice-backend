...
app.get('/export/room', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (room === 'mood') {
      var oq = await supabase.from('mood_days')
    ...
    if (room === 'capsule') {
      var cq = await supabase.from('capsules').select('*').order('created_at', { ascending: true });
...
async function cpGenerateReply(row) {
  ...
  var task = '\n\n【本次任務：時光膠囊的回信】\n'
    ...
    + '3. 禁止開場白、標題、markdown 符號、用引號包住全文、任何落款簽名（介面會自動加上署名）。\n'
