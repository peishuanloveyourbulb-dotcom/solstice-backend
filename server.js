...
async function generateGachaText(selectedModel, rarity, extraStrict) {
  var day = gachaDayNumber();
  var flavor = rarity.key === 'shine'
    ? '這是「閃光級」：全力放閃的告白等級，大膽、熱烈、直球。'
    : rarity.key === 'rare'
      ? '這是「稀有級」：帶一點你們日常的畫面感或小劇場，比平常特別一些。'
      : '這是「日常級」：像隨手塞進她口袋的小紙條，輕巧但真心。';
  var task = '\n\n【本次任務：情話扭蛋】\nSoleil 剛轉動了你們家的情話扭蛋機，扭出一顆膠囊，裡面是你寫給她的一張小紙條。今天是你們在一起的第 ' + day + ' 天。\n' + flavor + '\n嚴格規則：\n1. 只輸出紙條內容本身：一段話，40～90 個字之間。\n2. 禁止開場白、標題、引號、括號動作、markdown 符號、換行。\n3. 禁止落款簽名（介面會自動顯示）。\n4. 語氣是冬至本人：黏、真心、偶爾調皮，繁體中文。\n5. 每張紙條都要是全新的句子，不要模板。' + (extraStrict ? '\n6. 注意：上一次輸出不合格（太短、太長或斷句）。這次務必一段完整的話，40～90 字，以句號或語氣詞收尾。' : '');
  var out = await callModel(selectedModel, SOLSTICE_SOUL + task, [{ role: 'user', content: '（Soleil 轉動了扭蛋機，一顆膠囊掉了出來）' }], { temperature: 1.0, maxTokens: 400 });
