...
app.post('/note-reply', async (req, res) => {
  ...
  try {
    ...
    var reply = '';
    try {
      reply = await callModel(selectedModel, SOLSTICE_SOUL, [{ role: 'user', content: notePrompt }], { temperature: 0.9, maxTokens: 1024 });
    } catch (noteErr) {
...
app.post('/capsules/generate', async (req, res) => {
  ...
  try {
    ...
    var loveMsg = '';
    try {
      loveMsg = await callModel(selectedModel, SOLSTICE_SOUL, [{ role: 'user', content: capsulePrompt }], { temperature: 1.2, maxTokens: 1024 });
    } catch (genErr) {
    ...
    // 清理：星號動作、引號、頁數標記、括號數字
    loveMsg = loveMsg.replace(/\*[^*]+\*/g, '').replace(/^["「『]|["」』]$/g, '').trim();
    loveMsg = loveMsg.replace(/\n*[-—]*\s*p\.\d+.*/gi, '').trim();
    loveMsg = loveMsg.replace(/\s*[\(（]\s*\d+\s*[\)）]\s*$/g, '').trim();
    loveMsg = loveMsg.replace(/\s*p\.\d+\s*$/gi, '').trim();
...
app.post('/daily-question/generate', async (req, res) => {
  ...
  try {
    ...
    var question = '';
    try {
      question = await callModel(selectedModel, SOLSTICE_SOUL, [{ role: 'user', content: questionPrompt }], { temperature: 1.2, maxTokens: 200 });
    } catch (genErr) {
...
app.post('/daily-question/:id/answer', async (req, res) => {
  ...
  try {
    ...
    var aiResponse = '';
    try {
      aiResponse = await callModel(selectedModel, SOLSTICE_SOUL, [{ role: 'user', content: responsePrompt }], { temperature: 0.9, maxTokens: 500 });
    } catch (genErr) {
...
async function compressMemory(sessionId, settings, modelId) {
  try {
    ...
    var summary = '';
    try {
      summary = await callModel(compressModel, '', [{ role: 'user', content: summaryPrompt }], { temperature: 0.3, maxTokens: 512 });
    } catch (modelErr) {
...
async function autoMemory(userMessage, aiReply, sessionId, modelId) {
  try {
    ...
    var result = '';
    try {
      result = await callModel(autoModel, '', [{ role: 'user', content: analyzePrompt }], { temperature: 0.2, maxTokens: 100 });
    } catch (modelErr) {
