    ...
    if (room_id && chatMode === 'normal') {
      try {
        var { count: sgMsgCount } = await supabase.from('custom_messages')
          ...
          .eq('room_id', room_id)
          .eq('session_group', sg);
        // 每 4 筆 = 每 2 輪對話（user+assistant 各一筆）觸發一次
        if (sgMsgCount && sgMsgCount % 4 === 0) {
        ...
        console.log('[CustomChat] Room', room_id, 'sg', sg, 'msgCount:', sgMsgCount, 'trigger:', !!(sgMsgCount && sgMsgCount % 4 === 0));
