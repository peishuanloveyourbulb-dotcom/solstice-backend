...
app.post('/heartbeat/generate', async (req, res) => {
  try {
    ...
    function sampleAcrossSessions(msgs, total) {
      ...
      var keys = hbShuffle(Object.keys(bySession));
      var picked = [];
      for (var si = 0; si < keys.length && picked.length < total; si++) {
        var pool = bySession[keys[si]];
        var chunk = Math.min(pool.length, Math.max(2, Math.ceil(total / keys.length)));
        var startIdx = Math.floor(Math.random() * Math.max(1, pool.length - chunk + 1));
        picked = picked.concat(pool.slice(startIdx, startIdx + chunk));
      }
      picked.sort(function(a, b) { return new Date(a.created_at) - new Date(b.created_at); });
      return picked.slice(-total);
    }
    ...
    if (hoursSince <= 24) {
      ...
      var aMems = hbSample(pinnedMems, 2);
