...
app.patch('/sessions/:id', async (req, res) => {
  try {
    ...
    if (pinned !== undefined) {
      ...
      if (pinned) updates.updated_at = new Date().toISOString();  // only bump time when pinning; unpin returns to natural position
    }
...
app.patch('/memories/:id', async (req, res) => {
  try {
    ...
    if (pinned !== undefined) {
      updates.pinned = pinned;
      updates.pinned_at = pinned ? new Date().toISOString() : null;
      if (pinned) updates.edited_at = new Date().toISOString();  // pin = bump to top; unpin = keep natural time position
    }
