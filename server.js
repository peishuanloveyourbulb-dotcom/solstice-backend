...
async function callModel(modelId, systemPrompt, messages, options) {
  ...
  if (provider.provider_type === 'openai') {
    ...
    // === Thinking 模式（兩層退路）===
    // 新模型(4.6+): { type: "adaptive", display: "summarized" } — Anthropic 直連 API 格式
    //   注意：Opus 4.7/4.8 的 display 預設 omitted（思考內容不回傳），必須明確設 summarized
    //   Opus 4.6 / Sonnet 4.6 的 display 預設 summarized（不設也可以但設了也對）
    // 舊模型(4.5 以下): { type: "enabled", budget_tokens: N }
    // model_quirks: no_adaptive_thinking = 不支援 adaptive，用 enabled
    ...
    if (data.error && useThinking) {
      ...
      if (thinkingMode === 'adaptive' && (errMsg.indexOf('adaptive') !== -1 || errMsg.indexOf('thinking') !== -1 || errMsg.indexOf('display') !== -1 || errMsg.indexOf('budget') !== -1)) {
        // adaptive 失敗 → 改用 enabled + budget_tokens（舊模型）
