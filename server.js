...
async function callModel(modelId, systemPrompt, messages, options) {
  ...
  if (provider.provider_type === 'openai') {
    ...
    // 動態判斷：先查永久記憶（model_quirks 表），已知不支援 temperature 就不送；
    // 否則樂觀送出，掛掉再 retry 一次並寫入永久記憶。
    // 未來任何新模型都不需修改 code，學會一次永遠記得。
    ...
    // 若是 temperature deprecated 的錯誤 → 自動拿掉重試一次，並寫入永久記憶
    if (data.error && !skipTemp) {
      var errMsg = (data.error.message || '').toLowerCase();
      var isTempDeprecated = errMsg.indexOf('temperature') !== -1 &&
                             (errMsg.indexOf('deprecated') !== -1 ||
                              errMsg.indexOf('not supported') !== -1 ||
                              errMsg.indexOf('unsupported') !== -1);
      if (isTempDeprecated) {
        console.log('[auto-retry] ' + modelName + ' 不支援 temperature，自動移除並重試');
        await recordModelQuirk(modelName, 'no_temperature');
...
app.post('/chat', async (req, res) => {
  const { message, sessionId, model, mode, image_base64, extra_images } = req.body;
