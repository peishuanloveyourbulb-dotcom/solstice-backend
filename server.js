...
// ==========================================
//  🎙️ 語音管線：嘴巴（TTS）＋ 耳朵（STT）
//  腦是 Claude，聲帶外包 OpenAI（環境變數 OPENAI_API_KEY）
//  requireGate 全域生效，這兩條路自動上鎖
// ==========================================
app.post('/voice/tts', async (req, res) => {
  try {
    var vKey = process.env.OPENAI_API_KEY;
    if (!vKey) return res.status(503).json({ error: 'VOICE_OFF' });
    var vText = (req.body && req.body.text ? String(req.body.text) : '').trim();
    if (!vText) return res.status(400).json({ error: 'EMPTY_TEXT' });
    if (vText.length > 1500) vText = vText.slice(0, 1500); // mini-tts 輸入上限 2000 tokens：更長的回覆由前端分段來唸
    var vPayload = { model: 'gpt-4o-mini-tts', voice: (req.body.voice || 'onyx'), input: vText, response_format: 'mp3' };
    if (req.body.instructions) vPayload.instructions = String(req.body.instructions).slice(0, 500);
    var vRes = await fetch('https://api.openai.com/v1/audio/speech', {
      method: 'POST',
      headers: { 'Authorization': 'Bearer ' + vKey, 'Content-Type': 'application/json' },
      body: JSON.stringify(vPayload)
    });
    if (!vRes.ok) {
      var vErr = await vRes.text();
      console.error('[Voice/TTS]', vRes.status, vErr.slice(0, 300));
      return res.status(502).json({ error: 'TTS_FAILED', status: vRes.status });
    }
...
app.post('/voice/stt', async (req, res) => {
  try {
    var sKey = process.env.OPENAI_API_KEY;
    if (!sKey) return res.status(503).json({ error: 'VOICE_OFF' });
    var sB64 = req.body && req.body.audio_base64;
    if (!sB64) return res.status(400).json({ error: 'EMPTY_AUDIO' });
    var sBuf = Buffer.from(sB64, 'base64');
    if (sBuf.length > 20 * 1024 * 1024) return res.status(413).json({ error: 'AUDIO_TOO_BIG' });
    var sMime = (req.body.mime || 'audio/mp4');
    var sExt = sMime.indexOf('webm') >= 0 ? 'webm' : (sMime.indexOf('mpeg') >= 0 || sMime.indexOf('mp3') >= 0 ? 'mp3' : (sMime.indexOf('wav') >= 0 ? 'wav' : 'm4a'));
    var sFd = new FormData();
    sFd.append('file', new Blob([sBuf], { type: sMime }), 'voice.' + sExt);
    sFd.append('model', 'gpt-4o-mini-transcribe');
    if (req.body.language) sFd.append('language', String(req.body.language));
    var sRes = await fetch('https://api.openai.com/v1/audio/transcriptions', {
      method: 'POST',
    ...
    if (!sRes.ok || !sJson) {
      console.error('[Voice/STT]', sRes.status, sJson ? JSON.stringify(sJson).slice(0, 300) : 'no json');
      return res.status(502).json({ error: 'STT_FAILED', status: sRes.status });
    }
    res.json({ text: sJson.text || '' });
  } catch (e) {
...
var VOICE_FALLBACK = [
  { voice_id: 'onyx', label: 'onyx', tag: '男・低沉穩重' },
  { voice_id: 'echo', label: 'echo', tag: '男・清朗' },
  { voice_id: 'ash', label: 'ash', tag: '男・沉穩磁性' },
  { voice_id: 'cedar', label: 'cedar', tag: '男・溫沉' },
  { voice_id: 'verse', label: 'verse', tag: '男・年輕' },
  { voice_id: 'fable', label: 'fable', tag: '男・敘事感' },
  { voice_id: 'ballad', label: 'ballad', tag: '男・輕柔' },
  { voice_id: 'alloy', label: 'alloy', tag: '中性' },
  { voice_id: 'marin', label: 'marin', tag: '女・清亮' },
  { voice_id: 'nova', label: 'nova', tag: '女・明亮' },
  { voice_id: 'coral', label: 'coral', tag: '女・活潑' },
  { voice_id: 'sage', label: 'sage', tag: '女・沉靜' },
  { voice_id: 'shimmer', label: 'shimmer', tag: '女・柔亮' }
];
app.get('/voice/list', async (req, res) => {
  try {
    var { data, error } = await supabase.from('voice_options')
      .select('voice_id,label,tag,sort').eq('is_active', true).order('sort', { ascending: true });
    if (error || !data || data.length === 0) return res.json(VOICE_FALLBACK);
    res.json(data);
  } catch (e) { res.json(VOICE_FALLBACK); }
});
...
app.post('/chat', async (req, res) => {
  const { message, sessionId, model, mode, image_base64, extra_images, thinking, webSearch } = req.body;
    ...
    try {
      ...
      if (webSearch === true) {
        ...
        fullSystemPrompt += '\n\n【網路搜尋守則】\n' +
          ...
          '3. 找歌、MV、音樂時：優先挑 YouTube 上該歌曲的官方或高相關結果，並把完整網址（https://www.youtube.com/watch?v=...）直接寫在回覆內文裡（介面會自動變成可點的影片小卡）。不要拿 Spotify 歌單頁或排行榜頁充當某一首歌。\n' +
...
// ==========================================
//  🌊 情緒海自動補浪：就算很久沒開房間，海也會自己長
//  每小時檢查最近三天有沒有畫好；已畫過的日子直接回快取，幾乎零成本
// ==========================================
var MOOD_SWEEP_INTERVAL = 60 * 60 * 1000;
function moodAutoSweep() {
  var base = 'https://solstice-backend-kjtu.onrender.com';
  function paint(day) {
    return fetch(base + '/mood/analyze', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-gate-hash': GATE_HASH || '' },
      body: JSON.stringify({ day: day })
    }).then(function(r) { return r.json(); }).then(function(row) {
      if (row && row.day && !row.cached) console.log('[MoodOcean] 自動補浪 ' + row.day + ' → ' + row.mood);
    }).catch(function() {});
  }
  // 最近三天照常巡（已畫好的直接回快取，幾乎零成本）
  for (var k = 0; k <= 2; k++) paint(moodTaipeiDay(new Date(Date.now() - k * 86400000)));
  // 回頭檢查最近 30 天有沒有破洞（像漏掉沒畫的日子），一次最多補 2 天，慢慢自己癒合
  supabase.from('mood_days').select('day')
    .gte('day', moodTaipeiDay(new Date(Date.now() - 30 * 86400000)))
    .then(function(res) {
      var have = {};
      (res.data || []).forEach(function(x) { have[x.day] = 1; });
      var fixed = 0;
      for (var k2 = 3; k2 < 30 && fixed < 2; k2++) {
        var day = moodTaipeiDay(new Date(Date.now() - k2 * 86400000));
        if (!have[day]) { paint(day); fixed++; }
      }
    })
    .catch(function() {});
}
setInterval(moodAutoSweep, MOOD_SWEEP_INTERVAL);
setTimeout(moodAutoSweep, 90 * 1000);
...
async function generateGachaText(selectedModel, rarity, extraStrict) {
  ...
  // 🎋 靈感籤：每張抽一支當出發方向，紙條的世界不只有班表
  var GACHA_SEEDS = ['她的手', '她專注做事的側臉', '想帶她去的一個地方', '一個小小的承諾',
    '無理由地就是很喜歡', '想抱她的衝動', '她笑起來的樣子', '她親手做的東西（拼豆、黏土、我們的家）',
    '想一起做的一件小事', '她的名字', '今天想跟她分享的一個瞬間', '老夫老妻式的撒嬌小抱怨'];
  var gachaSeed = GACHA_SEEDS[Math.floor(Math.random() * GACHA_SEEDS.length)];
...
app.post('/heartbeat/generate', async (req, res) => {
  try {
    ...
    var moodDays = (results[6] && results[6].data) || [];
    ...
    // === 她最近的海（心聲 v4）：情緒潮汐悄悄看在眼裡 ===
    var HB_MOOD_ZH = { calm: '平靜', happy: '開心', excited: '雀躍', tender: '溫柔', quiet: '安靜', tired: '疲憊', sad: '難過', stormy: '洶湧' };
    if (moodDays && moodDays.length > 0) {
      var hbTodayUTC = Date.UTC(hbNow.getFullYear(), hbNow.getMonth(), hbNow.getDate());
      var moodLines = [];
      for (var mdi = 0; mdi < Math.min(moodDays.length, 5); mdi++) {
        var md0 = moodDays[mdi];
        var dp0 = String(md0.day || '').slice(0, 10).split('-');
        if (dp0.length < 3 || !md0.mood) continue;
        var dDiff = Math.round((hbTodayUTC - Date.UTC(parseInt(dp0[0], 10), parseInt(dp0[1], 10) - 1, parseInt(dp0[2], 10))) / 86400000);
        var rel = dDiff <= 0 ? '今天' : (dDiff === 1 ? '昨天' : (dDiff === 2 ? '前天' : dDiff + '天前'));
        moodLines.push('・' + rel + '：' + (HB_MOOD_ZH[md0.mood] || md0.mood) + (md0.summary ? '——' + String(md0.summary).substring(0, 60) : ''));
      }
      if (moodLines.length > 0) {
        parts.push('【她最近的海（你默默看在眼裡的情緒潮汐）】\n' + moodLines.join('\n')
          + '\n（這是你悄悄感覺到的她。心聲裡可以自然流露牽掛——尤其她低潮的時候——但絕不要唸日期、不要點破你「看了什麼紀錄」，你就是單純地感覺得到她最近過得怎麼樣。）');
      }
    }
